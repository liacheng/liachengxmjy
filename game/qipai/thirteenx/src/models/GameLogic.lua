...
GameLogic.CT_FIVE_MIXED_FLUSH_FIRST_A		= 5								--A前顺子
GameLogic.CT_FIVE_MIXED_FLUSH_NO_A			= 6								--普通顺子
...
GameLogic.CT_FIVE_STRAIGHT_FLUSH_FIRST_A	= 11							--A同花顺
GameLogic.CT_FIVE_STRAIGHT_FLUSH			= 12							--同花顺牌
	...
	elseif bCardCount == 5 then
		...
		elseif true == bFlushNoA then
			...
			--同花顺类型
			 	...
			 	return GameLogic.CT_FIVE_STRAIGHT_FLUSH, bMaxCardData 
			end
	...
	if true == bComperWithOther then
		if cbNextType == cbFirstType then
			...
			elseif cbFirstType == GameLogic.CT_FIVE_STRAIGHT_FLUSH_FIRST_A or 
				cbFirstType == GameLogic.CT_FIVE_STRAIGHT_FLUSH then 	--A在前同花顺,同花顺牌
				if cbFirstType == GameLogic.CT_FIVE_STRAIGHT_FLUSH_FIRST_A then
