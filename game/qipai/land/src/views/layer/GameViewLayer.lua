...
function GameViewLayer:priGameLayerZorder()
    return TAG_ZORDER.PRI_INFO_ZORDER
end
...
function GameViewLayer:loadResource()
    ...
    --播放背景音乐
    ExternalFun.setBackgroundAudio("background.mp3")
   ...
   self:registerScriptTouchHandler(function(eventType, x, y)
     ...
     --菜单按钮
	local btnMenu = csbNode:getChildByName("Button_down")
    btnMenu:setTag(TAG_ENUM.BT_MENU)
	btnMenu:addTouchEventListener(btnEvent)
       
    --菜单背景 
    self.m_AreaMenu = csbNode:getChildByName("menu")
        :setScale(0)
        :setLocalZOrder(5)
    --返回按钮
	local btn_back = self.m_AreaMenu:getChildByName("btn_back")
	btn_back:setTag(TAG_ENUM.BT_BACK)
	btn_back:addTouchEventListener(btnEvent)

    --换桌按钮
	local btnChangetable = self.m_AreaMenu:getChildByName("btn_changetable")
	btnChangetable:setTag(TAG_ENUM.BT_CHANGETABLE)
	btnChangetable:addTouchEventListener(btnEvent)

    --帮助按钮
	local btnHelp = self.m_AreaMenu:getChildByName("btn_help")
	btnHelp:setTag(TAG_ENUM.BT_HELP)
	btnHelp:addTouchEventListener(btnEvent)

    --设置按钮
	local btnSetting = self.m_AreaMenu:getChildByName("btn_set")
	btnSetting:setTag(TAG_ENUM.BT_SETS)
	btnSetting:addTouchEventListener(btnEvent)
...
--    btn:addTouchEventListener(btnEvent)
    ...
    for i = 1, 3 do
        ...
        self.m_tabReadySp[i]:setPosition(tabOkPosition[i])
    ...
    --调试扑克
--    local zz = {[0]=1,[1]=1}
--     self.m_tabNodeCards[cmd.MY_VIEWID]:updateCardsNode(zz, true, true, pCallBack)
    ...
    --用户昵称
    self.m_clipNick = ClipText:createClipText(cc.size(200, 30), GlobalUserItem.tabAccountInfo.szNickName, appdf.FONT_FILE, 30)
    local tmp = infoLayout:getChildByName("tmpname_text")

    local strName = infoLayout:getChildByName("strName")
    strName:setString(self:getParentNode():GetMeUserItem().szNickName)
    ...
    -- 头像位置
    self.m_tabUserHeadPos[cmd.LEFT_VIEWID] = cc.p(192, 526)
    self.m_tabUserHeadPos[cmd.MY_VIEWID] = cc.p(192, 308)
    self.m_tabUserHeadPos[cmd.RIGHT_VIEWID] = cc.p(1150, 526)

    -- 提示tip
    self.m_spInfoTip = self.m_onGameControl:getChildByName("info_tip")

    -- 倒计时
    self.m_spTimer = infoLayout:getChildByName("bg_clock")
    self.m_atlasTimer = self.m_spTimer:getChildByName("atlas_time")
    self.m_atlasTimer1= self.m_spTimer:getChildByName("atlas_time1")
...
-- 重置
function GameViewLayer:ResetView(state)
    -- 清理桌面
    self.m_callScoreControl:setVisible(false)
    self.m_onGameControl:setVisible(false)
    self.m_trusteeshipControl:setVisible(false)
    ...
    self.m_atlasTimer1:setString("")
    
    self.m_bMyCallBanker = false
    self.m_bMyOutCards = false

    if state=='Conclude' then
    ...
    elseif state=='Ready' or state=='NewGame' then
        -- 清理手牌
        for k,v in pairs(self.m_tabNodeCards) do
            v:removeAllCards()
            if v.m_bakPosition then
                v:setPosition(v.m_bakPosition[1], v.m_bakPosition[2])
                v.m_bakPosition = nil
            end
    ...
    elseif state=='ReInit' or state=='Reset' then
        self:ResetView('Ready')
        for k,v in pairs(self.m_tabUserHead) do
            self:OnUpdateUser(k, nil, true)
        end
    end
end

function GameViewLayer:onResetView()
    self:ResetView('ReInit')
end
...
function GameViewLayer:onButtonClickedEvent(tag, ref)   
    ...
    elseif TAG_ENUM.BT_READY == tag then        --准备
        if bMenuInOutside then self:onButtonSwitchAnimate() end
        self:onClickReady()
    ...
    elseif TAG_ENUM.BT_OUTCARD == tag then      --出牌
     ...
     elseif TAG_ENUM.BT_SETS == tag then        --设置 
        if bMenuInOutside then self:onButtonSwitchAnimate() end
        local set = SettingLayer:create()
        self:addToRootLayer(set, TAG_ZORDER.SET_ZORDER)
    elseif  TAG_ENUM.BT_CHANGETABLE == tag then --换桌
...
--	        end
    ...
    elseif  TAG_ENUM.BT_HELP == tag  then       --帮助
        if bMenuInOutside then self:onButtonSwitchAnimate() end
    ...
    elseif TAG_ENUM.BT_BACK == tag then         --退出
        if bMenuInOutside then self:onButtonSwitchAnimate() end
        
        self:getParentNode():onQueryExitGame()

    elseif TAG_ENUM.BT_MENU == tag then        --下拉菜单
...
function GameViewLayer:onButtonSwitchAnimate()
  ...
  --  printf("-- event: %s", self.bMenuInOutside)
--    printf("dddddddd"..self.bMenuInOutside)
--     printf("dddddddd"..self.fScale)
    self.m_AreaMenu:runAction(cc.ScaleTo:create(fSpeed, fScale))
end
...
function GameViewLayer:updateClock( viewid, clockId, cbTime)

    self:OnUpdataTimeView(cbTime)
    if cbTime <= 0 then
        if cmd.TAG_COUNTDOWN_READY == clockId then
            --退出防作弊
            self:getParentNode():getFrame():setEnterAntiCheatRoom(false)
        elseif cmd.TAG_COUNTDOWN_CALLSCORE == clockId and viewid==cmd.MY_VIEWID then
            -- 私人房无自动托管
            if not GlobalUserItem.bPrivateRoom then
                self:onGameTrusteeship(true)
            end            
        elseif cmd.TAG_COUNTDOWN_OUTCARD == clockId and viewid==cmd.MY_VIEWID then
            -- 私人房无自动托管
            if not GlobalUserItem.bPrivateRoom then
                self:onGameTrusteeship(true)
            end
...
--更新时钟的显示
-- @param  cbTime  时间
function GameViewLayer:OnUpdataTimeView( cbTime )
    if cbTime <= 0 then return end 
    self.m_atlasTimer:setString( string.format("%02d", cbTime ))
    self.m_atlasTimer1:setString( string.format("%02d", cbTime ))
    self.m_atlasTimer1:setScale(1)
    self.m_atlasTimer1:setOpacity(255)
    self.m_atlasTimer1:runAction(cc.Spawn:create(cc.ScaleTo:create(0.5, 2), cc.FadeOut:create(0.5)))
end
...
function GameViewLayer:onGetBankerInfo(bankerViewId, cbBankerScore, bankerCards, bReEnter)
    ...
    for k,v in pairs(self.m_tabUserHead) do
        ...
        self.m_tabStateSp[k]:setSpriteFrame( yl.GetPublicBlankFrame() )
    end
...
function GameViewLayer:onGetOutCard(curViewId, lastViewId, lastOutCards, bReEnter)
    ...
    elseif lastViewId==cmd.MY_VIEWID then
        self.m_onGameControl:setVisible(false)
    end
