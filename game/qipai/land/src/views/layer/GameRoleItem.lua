...
function GameRoleItem:ctor(userItem, viewId)
    ...
    head:setPositionX(head:getPositionX() - 80)
    self.m_popHead = head
    ...
    --头像底板
    local headbg = csbNode:getChildByName("Image_head")
    self.m_spHeadBg = headbg
    --玩家昵称
    local txt_name = headbg:getChildByName("txt_name")
    txt_name:setString(self.m_userItem.szNickName)
    --玩家金币
    local txt_jedata = headbg:getChildByName("txt_jedata")
    txt_jedata:setString(self.m_userItem.lScore)
    self.m_clipNick = nil
...
function GameRoleItem:reSet()
    ...
    self.m_spHeadBg:setVisible(true)
...
function GameRoleItem:textChat( str )
    ...
    if nil == self.m_labChat then
        self.m_labChat = cc.LabelTTF:create(str, appdf.FONT_FILE, 20, cc.size(150,0), cc.TEXT_ALIGNMENT_CENTER)        
