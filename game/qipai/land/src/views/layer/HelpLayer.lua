...
HelpLayer.RES_PATH = "game/qipai/land/res/"
function HelpLayer:ctor(scene, nKindId, nType)
    ...
    --加载csb资源
    local csbNode = ExternalFun.loadCSB(HelpLayer.RES_PATH.."help/HelpLayer.csb", self)
    local function btnEvent( sender, eventType )
	    if sender:getTag() == HelpLayer.BT_CLOSE then
    ...
    local csbBg = csbNode:getChildByName("img_panel")
    ExternalFun.showLayer(self, self, true, true,csbBg,false)

    --关闭按钮
    local btn = csbBg:getChildByName("btn_close")
    btn:setTag(HelpLayer.BT_CLOSE)
    btn:addTouchEventListener(btnEvent)

    local textBg = csbBg:getChildByName("help_textBg")
    -- 界面
    local tmp = textBg:getChildByName("help_text")
    -- 读取文本
...
function HelpLayer:refreshIntroduce(szTips)
    ...
    self._strLabel = cc.Label:createWithTTF(szTips, appdf.FONT_FILE, 25)
                             ...
                             :setTextColor(cc.c4b(120,77,44,255))
