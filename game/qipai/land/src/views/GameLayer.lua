...
function GameLayer:OnEventGameClockInfo(chair,time,clockId)
    if nil ~= self._gameView and nil ~= self._gameView.updateClock then
        self._gameView:updateClock(self:SwitchViewChairID(chair), clockId, time)
    end
end
...
function GameLayer:onSubGameConclude(dataBuffer)
    ...
    -- ui上最多放25张牌,超过移位Y一下(reSetForNewGame时重置)
    local otherUserChair = { ((self:GetMeChairID()+1)%3), ((self:GetMeChairID()+2)%3) }
    local otherUserCardCount = { countlist[ otherUserChair[1]+1 ] , countlist[ otherUserChair[2]+1 ] }
    if  otherUserCardCount[1]+otherUserCardCount[2] >= 25 then
        local nodeCards = nil
        if otherUserCardCount[1] > otherUserCardCount[2] then
            nodeCards = self._gameView.m_tabNodeCards[ self:SwitchViewChairID(otherUserChair[2]) ]
        else
            nodeCards = self._gameView.m_tabNodeCards[ self:SwitchViewChairID(otherUserChair[1]) ]
        end
        if not nodeCards.m_bakPosition then
            nodeCards.m_bakPosition = {nodeCards:getPosition()}
        end
        nodeCards:setPosition(nodeCards.m_bakPosition[1], nodeCards.m_bakPosition[2]+90)
    end
