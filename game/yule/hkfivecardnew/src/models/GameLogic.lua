...
function GameLogic:SortCardList( cbCardDataTable )
	...
	--2017.7.16 A8910J 是最小顺子（A当7点），10JQKA 是最大顺子（A当14点），以前默认A当14了，所以要判断A当7点为最小顺子的情况
	if cbSortValue[5] == 14 and cbSortValue[4] == 11 and cbSortValue[3] == 10 and cbSortValue[2] == 9 and cbSortValue[1] == 8 then
	
		--最小顺子，把A换为7点，放到最前
		local cbAColor = self:GetCardColor(cbCardDataTable[5]);
		local cbACardData = cbAColor * 0x10 + 0x7;
		cbCardDataTable[5] = cbCardDataTable[4];
		cbCardDataTable[4] = cbCardDataTable[3];
		cbCardDataTable[3] = cbCardDataTable[2];
		cbCardDataTable[2] = cbCardDataTable[1];
		cbCardDataTable[1] = cbACardData;
	end
end
