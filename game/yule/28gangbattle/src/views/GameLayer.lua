...
local GameViewLayer = appdf.req(appdf.GAME_SRC.."yule.28gangbattle.src.views.layer.GameViewLayer")
local GameFrame = appdf.req(appdf.GAME_SRC.."yule.28gangbattle.src.models.GameFrame")
...
function GameLayer:sendRequestBankInfo()
    local buffer = ExternalFun.create_netdata(GameServer_CMD.CMD_GR_C_QueryInsureInfoRequest)
	buffer:setcmdinfo(GameServer_CMD.MDM_GR_INSURE,GameServer_CMD.SUB_GR_QUERY_INSURE_INFO)
	buffer:pushbyte(GameServer_CMD.SUB_GR_QUERY_INSURE_INFO)
...
function GameLayer:onGameMessageSendRecord(dataBuffer)
	...
	for i=1,recordcount do
		...
		rec.bWinShangMen = dataBuffer:readbool()
		rec.bWinTianMen = dataBuffer:readbool()
		rec.bWinXiaMen = dataBuffer:readbool()
