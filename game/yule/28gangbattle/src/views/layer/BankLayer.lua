...
local module_pre = "game.yule.28gangbattle.src"
...
function BankLayer:ctor(parentNode)
    ...
    self.bg=appdf.getNodeByName(self.csbNode,"sp_bg")
	
    local txt = self.bg:getChildByName("txt_bankscore")
        :setFontName(appdf.FONT_FILE)
    text = self.bg:getChildByName("txt_myscore")
        :setFontName(appdf.FONT_FILE)
	self.curCoinLabel = self.bg:getChildByName("m_pTextMyScore")
        :setFontName(appdf.FONT_FILE)
    self.insureLabel = self.bg:getChildByName("m_pTextMyBank")
        :setFontName(appdf.FONT_FILE)
    self.txt_Min = self.bg:getChildByName("txt_min")
        :setFontName(appdf.FONT_FILE)
    self.txt_Max = self.bg:getChildByName("txt_max")
        :setFontName(appdf.FONT_FILE)
    self.m_pTakeMoney = self.bg:getChildByName("im_take")
    self.m_pTakePassWord = self.bg:getChildByName("im_pw")

     --金额输入
    local editHanlder = function(event,editbox)
		self:onEditEvent(event,editbox)
	end
	self.text_GiveScore = ccui.EditBox:create(cc.size(605,60),"")
		:move(430,25)
		:setFontName(appdf.FONT_FILE)
        ...
        :setPlaceholderFontColor(cc.c4b(215,166,97,255))
    ...
    self._btnGiveMax = self.bg:getChildByName("btn_max")
        ...
        :addTouchEventListener(btncallback)
    self._btnTransfer = self.bg:getChildByName("btn_sure")
        :setTag(BankLayer.BT_TRANSFER)
        :addTouchEventListener(btncallback)
    self._btnClose = self.bg:getChildByName("btn_close")
...
function BankLayer:onGetBankInfo(bankinfo)
    self.curCoinLabel:setString(GlobalUserItem.lUserScore)
    self.insureLabel:setString(GlobalUserItem.lUserInsure)
end
