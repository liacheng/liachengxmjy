...
function GameViewLayer:gameDataInit(  )
    ...
    ExternalFun.setBackgroundAudio("game_res/back_music.mp3")
end
...
function GameViewLayer:showUserInfo( useritem )
    ...
    --玩家昵称
    local nick =  g_var(ClipText):createClipText(cc.size(120, 22),useritem.szNickName,appdf.FONT_FILE,22);
    ...
    local coin =  cc.Label:createWithTTF(str, appdf.FONT_FILE, 24)
...
function GameViewLayer:popMenu(sender)
 ...
 setBtn:setPosition(cc.p(menuBG:getContentSize().width/2-90,menuBG:getContentSize().height/2))
 ...
 local helpBtn =  ccui.Button:create("game_res/BT_help_0.png","game_res/BT_help_1.png")
 helpBtn:setAnchorPoint(cc.p(0.5,0.5))
 helpBtn:setPosition(cc.p(menuBG:getContentSize().width/2,menuBG:getContentSize().height/2))
 helpBtn:addTouchEventListener(function(sender,eventType)
         if eventType == ccui.TouchEventType.ended then
            menuBtn:loadTextureNormal("game_res/BT_menu_0.png")
            menuview:removeFromParent()
            self:getParentNode():getParentNode():popHelpLayer2(g_var(cmd).KIND_ID,0)
         end
 ...
 back:setPosition(cc.p(menuBG:getContentSize().width/2+90,menuBG:getContentSize().height/2))
...
function GameViewLayer:setView()
  ...
  local verstr = mgr:getResVersion(401) or "0"
  verstr = "游戏版本:" .. appdf.BASE_C_VERSION .. "." .. verstr
  local txt_ver = csbnode:getChildByName("txt_version")
...
function GameViewLayer:showCoachView()
  ...
  if self._scene.m_nCoachRestarin > self._scene:GetMeUserItem().lScore then 
      showToast(cc.Director:getInstance():getRunningScene(),"你携带的游戏币少于指导费最低限制,请取款或者充值!",2)
      return
  end
  ...
  local nick = cc.Label:createWithTTF(str, appdf.FONT_FILE, 24)
  nickNode:addChild(nick)
...
function GameViewLayer:gobangAlert(str,nType,OKCallback,CancellCallback,target)
  ...
  btnClose:addTouchEventListener(function ( sender,eventType )
       if eventType == ccui.TouchEventType.ended then
              if nil ~= CancellCallback and nType~=1 then
                  CancellCallback() 
              end 
              alert:removeFromParent()
              self._cancellTime = 0
  ...
  --内容
  local content =cc.Label:createWithTTF(str, appdf.FONT_FILE, 24)
  ...
  if nType == 1 then
    --确定
      ...
      self._cancellLabel:setPosition(cc.p(btnDisAgree:getContentSize().width/2 + 50 , btnDisAgree:getContentSize().height/2+5))
...
function GameViewLayer:showGameEnd(data)
  ...
  local  MeNick  = g_var(ClipText):createClipText(cc.size(120, 22),meItem.szNickName,appdf.FONT_FILE,22);
  ...
  local  scoreMe = cc.Label:createWithTTF(string.format("%d", score), appdf.FONT_FILE, 24)
  ...
  local  otherNick = g_var(ClipText):createClipText(cc.size(120, 22),self._scene._otherNick,appdf.FONT_FILE,22);
  ...
  local  scoreOther = cc.Label:createWithTTF(string.format("%d", score), appdf.FONT_FILE, 24)
