...
function GameLayer:onEventGameSceneStatus(buffer)   
    ...
    local cmd_table = ExternalFun.read_netdata(g_var(cmd).CMD_S_StatusPlay, buffer, ExternalFun.DisableAssert)
...
function GameLayer:OnSubRegretResult(dataBuffer)
    ...
    local round = cmd_table.wRegretCount --(self.m_wPlaceUser == self.m_wRegretUser and 1 or 2)
...
function GameLayer:OnSubChessManual(dataBuffer)
    ...
    for i=1,math.floor(dataBuffer:getlen()/3) do
        local cmd_table  = ExternalFun.read_netdata(g_var(cmd).tagChessManual, dataBuffer, ExternalFun.DisableAssert)
