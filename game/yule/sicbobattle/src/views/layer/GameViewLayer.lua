...
function GameViewLayer:initUserInfo( csbNode )
	...
	local head = g_var(PopupInfoHead):createClipHead(self:getMeUserItem(), tmp:getContentSize().width-20,"game/head_bg.png")
...
function GameViewLayer:onGetBankInfo(bankinfo)
	...
	if nil ~= self.m_bankLayer then
		...
		--self:refreshBankScore(bankinfo)
	end
end
------
---------------------------------------------------------------------------------------
...
function GameViewLayer:gameDataInit(  )
    ...
    --播放背景音乐
    print("播放背景音乐")
    ExternalFun.setBackgroundAudio("BACK_GROUND.wav")
...
--刷新金币
function GameViewLayer:refreshBankScore()--userinfo)
