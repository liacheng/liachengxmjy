...
function GameLayer:ctor( frameEngine,scene )
    ...
    self.m_nMultiple = 1
    self.m_lUserMaxScore = 0  --玩家最大游戏币
...
function GameLayer:onEventGameSceneFree(buffer)    --空闲 
    ...
    self.m_lUserMaxScore = cmd_table.lUserMaxScore  --玩家游戏币
    ...
    self._gameView:ShowBankerInfo(info)
     ...
     self._gameView:RollApear(false)
...
function GameLayer:onEventGameSceneStatus(buffer)   
    ...
    if self._gameView.m_cbGameStatus == g_var(cmd).GS_GAME_END then
       ...
       clockType = g_var(cmd).GS_GAME_END
        ...
        if nil ~= userItem and self.m_wBankerUser ~= yl.INVALID_CHAIR then
    ...
    if  self._gameView.m_cbGameStatus == g_var(cmd).GS_PLACE_JETTON then
        ...
        ExternalFun.playSoundEffect("luxurycar_please_add_gold.mp3")
        self._gameView:playBackGroundMusic(self._gameView.m_cbGameStatus)
    end
...
function GameLayer:onSubJettonFail( dataBuffer )
    ...
    if cmd_table.wPlaceUser == self:GetMeUserItem().wChairID then
		local toastStr = string.format("当前区域可下注额度不足,下注%s失败!", tostring(cmd_table.lPlaceScore))    -- 提示下注失败
        showToast(cc.Director:getInstance():getRunningScene(), toastStr, 1)
...
function GameLayer:onSubGameEnd( dataBuffer ) --游戏结束
    ...
    local nick
    local userItem =  self._dataModle:getUserByChair(self:getUserList(),self.m_wBankerUser)
    local szBankerWinScore
    local szBankerScore
    
    if self.m_lBankerWinScore >= 0 then
        szBankerWinScore = "+"..ExternalFun.numberThousands(self.m_lBankerWinScore)
    else
        szBankerWinScore = ExternalFun.numberThousands(self.m_lBankerWinScore)
    end
    ...
    if self.m_wBankerUser == yl.INVALID_CHAIR then
        ...
        szBankerScore  = ExternalFun.numberThousands(lScore)
    end
   
    --庄家信息
    self._info = {nick,szBankerWinScore,szBankerScore,self.m_cbBankerTime,userItem}
...
function GameLayer:onSubGameRecord( dataBuffer )
    ...
    if recordCount >= 1 then
        for i=1,recordCount do
          local record = ExternalFun.read_netdata(g_var(cmd).tagServerGameRecord,dataBuffer, ExternalFun.DisableAssert)
