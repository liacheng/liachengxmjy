...
function BankerList:tableCellAtIndex( view, idx )
	...
	if nil == cell then
		...
		--玩家游戏币
		local coin =  g_var(ClipText):createClipText(cc.size(150, 20),ExternalFun.formatScoreText(useritem.lScore*self._dataModule._Multiple))  
		...
		local head = g_var(PopupInfoHead):createNormal(useritem, 47)
		...
		if nil ~= cell:getChildByTag(4) then
			...
			coin:setString(ExternalFun.formatScoreText(useritem.lScore*self._dataModule._Multiple))
		end
		...
		if nil ~= cell:getChildByTag(5) then
			...
			if nil ~= head then
				...
				head = g_var(PopupInfoHead):createNormal(useritem, 47)
				head:setPosition(cc.p(headBG:getContentSize().width/2,headBG:getContentSize().height/2))
				head:setTag(1)
				headBG:addChild(head)
