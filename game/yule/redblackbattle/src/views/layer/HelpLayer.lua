...
HelpLayer.RES_PATH = "game/yule/redblackbattle/res/"
function HelpLayer:ctor(scene, nKindId, nType)
    ...
    --加载csb资源
    local csbNode = ExternalFun.loadCSB(HelpLayer.RES_PATH.."HelpLayer.csb", self)
    ...
    -- 界面
    local tmp = csbBg:getChildByName("m_pTextHelp")
    -- 读取文本
...
function HelpLayer:refreshIntroduce(szTips)
    ...
    self._strLabel = cc.Label:createWithTTF(szTips, appdf.FONT_FILE, 25)
                             ...
                             :setTextColor(cc.c4b(120,54,18,255))
