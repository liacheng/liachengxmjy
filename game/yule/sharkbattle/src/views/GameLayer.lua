...
function GameLayer:ctor( frameEngine,scene )     
    ...
    self.gameData.clicklAddScore = 1
    self.gameData.lAddScore = 0
	--下注变量
...
function GameLayer:onEventGameMessage(sub,dataBuffer)
    ...
    elseif sub==cmd.SUB_S_SWITCT_NUM then --切换数量
        return self:OnSubSwitchNum(dataBuffer)
	end
end

function GameLayer:OnSubSwitchNum(dataBuffer)
    local pSwitchNum = ExternalFun.read_netdata(cmd.CMD_S_SwitchNum, dataBuffer)
    --for i=1,6 do
        self.gameData.lAddScore = pSwitchNum.lAddScore
    --end
...
function GameLayer:onEventGameScene(cbGameStatus,dataBuffer)
 	ithMsg=ithMsg+1
	print("ithMsg: ",ithMsg)
	ExternalFun.setBackgroundAudio( "GAME_FREE.wav" )
	...
	if cbGameStatus==cmd.GAME_SCENE_FREE then
		...
		--设置单位注
		self.gameData.nUnitNote = BET_NUMBER
		self.gameData.nCurrentNote = BET_NUMBER
	...
	elseif cbGameStatus==cmd.GAME_SCENE_END then
		...
		self._gameView:SetGameStatus(cbGameStatus) --PC端是设置为cmd.GAME_SCENE_BET
...
function GameLayer:OnSubGameStart(dataBuffer)
	...
	self._gameView:UpdateShowTotalScore(self:GetMeUserItem().lScore)
	--消息处理
...
function GameLayer:OnSubGameEnd(dataBuffer)
	 ExternalFun.setBackgroundAudio( "GAME_START.wav" )
...
function GameLayer:OnSubPlayBetFail(dataBuffer)
	...
	if  self:GetMeChairID() == pPlayBetFail.wChairID   then
		...
		self._gameView:UpdateShowTotalScore(self:GetMeUserItem().lScore-self.gameData.allPlayBet)
	end
end
...
function GameLayer:OnMessagePlayBet(sender)
	...
	self._gameView:UpdateShowTotalScore(meItem.lScore-self.gameData.allPlayBet)
...
--续压按钮消息
 function GameLayer:OnMessageOperationRenewal()
	...
	if temp>self:GetMeUserItem().lScore then
	...
	for i=1,cmd.ANIMAL_MAX do
		...
		cmddata:pushint(i-1)
		cmddata:pushscore(self.gameData.lPlayBet[i])
		self:SendData(cmd.SUB_C_PLAY_BET,cmddata) 
...
function GameLayer:OnMessageOperationSwitch(sender)
	...
	if sender.m_plusNote==true then
        if self.gameData.clicklAddScore < 6 then 
            self.gameData.clicklAddScore = self.gameData.clicklAddScore+1
        else
           return
        end    
		self.gameData.nUnitNote= math.min(self.gameData.lAddScore[1][self.gameData.clicklAddScore],cmd.MAX_UNITNOTE)
	else
        if self.gameData.clicklAddScore > 1 then 
           self.gameData.clicklAddScore = self.gameData.clicklAddScore-1
        else 
            return
        end
		self.gameData.nUnitNote= math.max(self.gameData.lAddScore[1][self.gameData.clicklAddScore],cmd.MIN_UNITNOTE)
	end
...
function GameLayer:onSocketInsureEvent( sub,dataBuffer )
    ...
    if sub == GameServer_CMD.SUB_GR_USER_INSURE_SUCCESS then
        ...
        GlobalUserItem.updateScore(cmd_table)
        self._gameView:UpdateShowTotalScore(self:GetMeUserItem().lScore-self.gameData.allPlayBet)
