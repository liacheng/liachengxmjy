...
function GameViewLayer:ctor(scene)
    ...
    self.menuBtn:addClickEventListener(function() self.menuBtn:loadTextureNormal("menuunfold.png")  this:addChild(MenuUnfoldLayer:create(self,menuX,menuY,menuHeight,self.menuBtn),popZorder)  end)
...
function GameViewLayer:initData()
    ...
    self:updatePlayShowPrizes(math.random(70000)) 
   	self:UpdateShowTotalScore(self._scene:GetMeUserItem().lScore)
...
function GameViewLayer:UpdateShowTotalScore(score)
	...
	self.totalScoreAtlas:setString(getNineDigits(self.lscore))
end
...
function GameViewLayer:OnUpdataClockView(clockViewChair,clockTime)
	...
	if self.m_cbGameStatus==cmd.GAME_SCENE_END then
		...
		self.countdownSp:setTexture("countdownend.png")
	...
	local function showAnims(self,i)
		...
		local function callback(showtime)
			...
			if i==1 then
				...
				self.m_lPlayShowWin =self.m_lPlayWin[1]+ self.m_lPlayWin[2]+self.m_lPlayPrizes
			end
...
function GameViewLayer:initBetBtns() --12个Button
	...
	for i=1,8 do --i表示动物种类
		...
		local betRatio=ccui.Text:create("",appdf.FONT_FILE,26)
			:addTo(ratBoard)
			:setPosition(boardWidth/2,67)
			:setColor(cc.c3b(18,234,248)) --hex 12eaf8

		local betNum=ccui.Text:create("",appdf.FONT_FILE,22)
			:addTo(ratBoard)
			:setPosition(boardWidth/2,41)
			:setColor(cc.c3b(203,231,255)) --hex cbe7ff

		local allBetNum=ccui.Text:create("",appdf.FONT_FILE,22)
			:addTo(ratBoard)
