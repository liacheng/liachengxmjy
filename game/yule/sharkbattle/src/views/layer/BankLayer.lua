...
function BankLayer:ctor(parentNode)
	...
	self.curCoinLabel=cc.LabelAtlas:create(GlobalUserItem.tabAccountInfo.lUserScore.."", "bankdigits1.png", 23, 31, string.byte("0")) 
    		:setAnchorPoint(0,0.5)
    		:move(535,409)
    		:addTo(self.bg)

    self.insureLabel=cc.LabelAtlas:create(GlobalUserItem.tabAccountInfo.lUserInsure.."", "bankdigits2.png", 23, 31, string.byte("0")) 
    		:setAnchorPoint(0,0.5)
    		:move(535,371)
    		:addTo(self.bg)
	...
	self.edit_Score = ccui.EditBox:create(cc.size(289,45), ccui.Scale9Sprite:create("bankshadow.png"))
		...
		:setMaxLength(13)
		:setFontColor(cc.c4b(205,205,205,255))
    ...
    assert(self._parentNode:getParent() and not tolua.isnull(self._parentNode:getParent() ) )
    print(self._parentNode:getParent().sendRequestBankInfo)
...
function BankLayer:onGetBankInfo(bankinfo)
    ...
    if self.m_textTips==nil or tolua.isnull(self.m_textTips) then
    	self.m_textTips=ccui.Text:create(str,appdf.FONT_FILE,22)
    					:addTo(self.bg)
    					:setPosition(cc.p(520,162))
                        :setTextColor(cc.c4b(0,172,210,255))
    else
