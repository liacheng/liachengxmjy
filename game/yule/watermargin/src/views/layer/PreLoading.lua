...
--					"game2/gold_action/gold.plist",
                    ...
                    "gameResultLayer/gameResult.plist"
				   }

    -- 2017.11.12 修改纹理加载为一帧一帧加载，不然内存会突然飙升，导致闪退
    local totalSource = #plists --1 
	...
	local function imageLoaded(texture)--texture
        ...
        if m_nImageOffset < totalSource then
            ...
            --加载PLIST
            cc.SpriteFrameCache:getInstance():addSpriteFrames(res_path..plists[m_nImageOffset])
            local dict = cc.FileUtils:getInstance():getValueMapFromFile(res_path..plists[m_nImageOffset])
    ...
    local function 	loadImages()
    	cc.Director:getInstance():getTextureCache():addImage(res_path.."game1/gameAction/game1_itemCommon.png")
    end
