...
function Game1ViewLayer:ctor(scene)
    ...
    --播放背景音乐
    ExternalFun.setBackgroundAudio("watermargin_xiongdiwushu.mp3")
end
...
function Game1ViewLayer:addPath( )
    ...
    self._gameAddSearchPath = ExternalFun.addSearchPath({
        GameViewLayer.RES_PATH,
        GameViewLayer.RES_PATH .. "game1/",
        GameViewLayer.RES_PATH .. "game2/",
        GameViewLayer.RES_PATH .. "game3/",
        
        GameViewLayer.RES_PATH .. "common/",
        GameViewLayer.RES_PATH .. "setting/",
        GameViewLayer.RES_PATH .. "sound_res/",
    })
end
...
function Game1ViewLayer:initUI( csbNode )
	...
	------
    local Button_Chat = csbNode:getChildByName("game1_chat")
	Button_Chat:setTag(TAG_ENUM.TAG_CHAT)
	Button_Chat:addTouchEventListener(btnEvent)
    ...
    self.m_textTips:setFontName("fonts/round_body.ttf")
    ...
    --赢分
    self.Node_win = csbNode:getChildByName("Node_win")
    --self.Node_win:setPosition(cc.p(self.Node_win:getPositionX()+(yl.WIDTH - yl.DESIGN_WIDTH)/2, self.Node_win:getPositionY()))
end
...
function Game1ViewLayer:game1Begin(  )
    ...
    for i=1,15 do
        ...
        if node ~= nil then
			...
			if pItem then
                ...
                pItem:beginMove(i*0.1) --滚动延迟+时间差
                if i == 15 then
    ...
    ExternalFun.playSoundEffect("watermargin_gundong.mp3")
    self:runAction(
    	cc.Sequence:create(
...
function Game1ViewLayer:game1End(  )
    ...
    self:game1GetLineResult(true)
	--切换旗帜动作
	self:game1ActionBanner(false)
end
--游戏1结果
function Game1ViewLayer:game1Result(bQuickEnd)
        			...
        			if self._scene.tagActionOneKaiJian.bZhongJiang[posy][posx] == true then 
        				...
        				self:runAction(
        					cc.Sequence:create(
        						...
        						cc.DelayTime:create(bQuickEnd and 0 or 0.0),
        						cc.CallFunc:create(function (  )
                                    ...
                                    if self._scene.m_lGetCoins > 0 then
                                        self.m_WinScore = self._scene.m_lGetCoins
                                        self:showWinScore(self.m_WinScore)
                                    end
...
--										labNum:setPosition(667-(nLen-350)/2,375)
...
--        							end
    ...
    --切换按钮状态
    if self._scene.m_bIsAuto == false and self._scene.m_bEnterGame3 == false and self._scene.m_lGetCoins > 0 then
        self:updateStartButtonState(3)      --收分
    else
        self:updateStartButtonState(1)      --开始
    end
    ...
    local fTime = 0.5
    if self._scene.m_lGetCoins > 0 then
    	fTime = bQuickEnd and 0.5 or 2.0
    end
    --即将进入小玛丽
    				...
    				cc.Sequence:create(
    					...
    					cc.CallFunc:create(function (  )
                            if  self._scene.m_bIsAuto == false then
                                ...
                                --2017.12.06 自动模式不允许比倍
                                self:enableGame2Btn(false)
                            end
    						self._scene.m_bIsItemMove = false
    						--游戏模式
                            if self._scene:getGameMode() == 3 then
...
--游戏连线结果
function Game1ViewLayer:game1GetLineResult( bQuickEnd )
	print("游戏连线结果")
    self._scene:setGameMode(3)  --GAME_STATE_RESULT
    self.m_textGetScore:setString(self._scene.m_lGetCoins)
    --画中奖线
			...
			if pActionOneYaXian then
				self:runAction(
					cc.Sequence:create(
						...
						cc.CallFunc:create(function ()
							--音效
                            ExternalFun.playSoundEffect("watermargin_gundong_1.mp3")
							--如果是最后一个，进入结算界面
						        			...
						        			if pos.x == posy and pos.y == posx then
												...
												local action2 = cc.Animate:create(cc.AnimationCache:getInstance():getAnimation("lightAnim"))
											 	...
											 	nodeAct:runAction(cc.Sequence:create(
												 	cc.DelayTime:create(bQuickEnd and 1.0 or 1.5),
												 	cc.CallFunc:create(function (  )
...
function Game1ViewLayer:onButtonClickedEvent(tag,ref)
	if tag == TAG_ENUM.TAG_QUIT_MENU then  			--退出
        local posY = self.m_nodeMenu:getPositionY()
        if posY == yl.HEIGHT-95/2 then
	...
	elseif tag == TAG_ENUM.TAG_MAXADD_BTN  then    --	最大加注
        ...
        --声音
        ExternalFun.playSoundEffect("watermargin_shangfen.mp3")
	elseif tag == TAG_ENUM.TAG_MINADD_BTN  then    --	最小减注
		self._scene:onAddMinScore()
        --声音
        ExternalFun.playSoundEffect("watermargin_shangfen1.mp3")
	...
	elseif tag == TAG_ENUM.TAG_AUTO_START_BTN  then   --自动游戏
        if self.m_bIsTakingScore ~= true then
	...
	elseif tag == TAG_ENUM.TAG_SHOWUP_BTN  then   --显示上部菜单
		self:onShowTopMenu()
        ExternalFun.playClickEffect()
    elseif tag == TAG_ENUM.TAG_CHAT then
        print("聊天按钮被点击")
...
function Game1ViewLayer:onHideTopMenu()
    if self.m_nodeMenu:getNumberOfRunningActions() > 0 then
        return
    end
	...
	local actMove = cc.MoveTo:create(0.3,cc.p(667,posY+95))
...
function Game1ViewLayer:onResultLayer(tbScore)
	local result = ResultLayer:create(tbScore)
    self._csbNode:addChild(result)
    result:setLocalZOrder(9)
end
...
--切换开始按钮、停止按钮、取分按钮的纹理
function Game1ViewLayer:updateStartButtonState( state )
    ...
    elseif state == 3 then
        Button_Start:loadTextureNormal("game1/game1_take_1.png")
        Button_Start:loadTexturePressed("game1/game1_take_2.png")        
    end
...
function Game1ViewLayer:Game1ZhongxianAudio( bIndex )
    local soundPath = 
    {
        "watermargin_winsound.mp3",
        "watermargin_winsound.mp3",
        "watermargin_winsound.mp3",
        "watermargin_luzhisheng.mp3",
        "watermargin_linchong.mp3",
        "watermargin_songjiang.mp3",
        "watermargin_titianxingdao.mp3",
        "watermargin_zhongyitang.mp3",
        "watermargin_shuihuzhuan3.mp3"
    }
    ExternalFun.playSoundEffect(soundPath[bIndex])
end

--显示得分
function Game1ViewLayer:showWinScore(winscore)
    ...
    if self.Node_win then
        --win精灵
        self.Node_win:setVisible(true)
        self.Node_win:setLocalZOrder(9999999)

        local spriteIcon = self.Node_win:getChildByName("Sprite_icon")
        local labelNum = self.Node_win:getChildByTag(1)

        if labelNum == null then
            local endScoreStr =  "zhongjiang_num.png"
            labelNum = cc.LabelAtlas:_create("0",GameViewLayer.RES_PATH.."game1/" .. endScoreStr,101,101,string.byte("0"))
            labelNum:setAnchorPoint(cc.p(0.5,0.5))
            self.Node_win:addChild(labelNum, 0, 1)
        end

        --显示中奖分数
        labelNum:setString(winscore)

        --调整位置
        local iconWidth = spriteIcon:getContentSize().width
		local scoreWidth = labelNum:getContentSize().width
        local iconX = yl.DESIGN_WIDTH / 2 - (scoreWidth / 2 + 10)

        spriteIcon:setPosition(iconX, 375)
		labelNum:setPosition(iconX + (iconWidth + scoreWidth) / 2 + 20, 375)
		
		--隐藏
--	    self.Node_win:runAction(cc.Sequence:create(
...
--隐藏得分
function Game1ViewLayer:hideWinScore()

    self.Node_win:setVisible(false)
    self.Node_win:stopAllActions()
end

--收分
function Game1ViewLayer:takeWinScore()

    if self.m_WinScore == nil or self.m_WinScore <= 0 then
        self:hideWinScore()
        --更新按钮状态为开始
        self:updateStartButtonState(1)    --开始
        return
    ...
    self.m_tmpWinScore = self.m_WinScore
    self.m_bIsTakingScore = true

    local totalframes = 10
    local frame = totalframes
    local delta = self.m_WinScore / totalframes

    schedule(self.Node_win, function()

        self.m_tmpWinScore = self.m_tmpWinScore - delta
        if self.m_tmpWinScore < delta then
            self.m_tmpWinScore = 0
        end

        --改变赢分
        self:showWinScore(self.m_tmpWinScore)

        --改变总分
        self._scene:changeUserScore(delta)
...
function Game2ViewLayer:ctor(scene)
 ...
 function Game2ViewLayer:onEnter()
     print("Game2ViewLayer.onEnter")
     ...
     Game2ViewLayer:readAnimation("dealer_common_0","dealerComAnim",8,0.1,1);
     Game2ViewLayer:readAnimation("left_common_", "leftComAnim",27,0.1,2);
     Game2ViewLayer:readAnimation("right_common_", "rightComAnim",25,0.5,2);
     Game2ViewLayer:readAnimation("desk_", "deskAnim",5,0.1,1);
     Game2ViewLayer:readAnimation("game2_Gold_", "goldAnim",4,0.1,1);

     Game2ViewLayer:readAnimation("dealer_dice_", "dealerDiceAnim",29,0.1,2);
     Game2ViewLayer:readAnimation("left_cheer_", "leftCheerAnim",29,0.1,2);
     Game2ViewLayer:readAnimation("right_cheer_", "rightCheerAnim",29,0.1,2);
     Game2ViewLayer:readAnimation("desk_", "deskAnim",5,0.1,1);
     Game2ViewLayer:readAnimation("game2_Gold_", "goldAnim",4,0.1,1);

     Game2ViewLayer:readAnimation("dealer_open_", "dealerOpenAnim",14,0.1,2);
     Game2ViewLayer:readAnimation("dealer_anger_", "dealerAngerAnim",25,0.1,2);
     Game2ViewLayer:readAnimation("dealer_happy_0", "dealerHappyAnim",7,0.3,1);

     Game2ViewLayer:readAnimation("left_happy_", "leftHappyAnim",55,0.1,2);
     Game2ViewLayer:readAnimation("left_cry_", "leftCryAnim",36,0.1,2);

     Game2ViewLayer:readAnimation("right_happy_", "rightHappyAnim",18,0.1,2);
     Game2ViewLayer:readAnimation("right_cry_", "rightCryAnim",26,0.1,2);
     --初始化等待画面
 ...
 function Game2ViewLayer:onExit(  )
     ...
     local plists2 = {
         "game2/dealer/dealer_common.png",
         "game2/dealer/dealer_anger1.png",
         "game2/dealer/dealer_anger2.png",
         "game2/dealer/dealer_cry.png",
         "game2/dealer/dealer_dice1.png",
         "game2/dealer/dealer_dice2.png",
         "game2/dealer/dealer_happy.png",
         "game2/dealer/dealer_open.png",

         "game2/left/left_cheer.png",
         "game2/left/left_common.png",
         "game2/left/left_cry.png",
         "game2/left/left_happy.png",

         "game2/right/right_cheer1.png",
         "game2/right/right_cheer2.png",
         "game2/right/right_common.png",
         "game2/right/right_common2.png",
         "game2/right/right_cry.png",
         "game2/right/right_happy.png",

         "game2/desk/desk.png",
         "game2/gold_action/gold.png"
     }

     for i=1,#plists do
         print("game2 onExit")
         cc.SpriteFrameCache:getInstance():removeSpriteFramesFromFile(res_path..plists[i])
         cc.Director:getInstance():getTextureCache():removeTextureForKey(res_path..plists2[i])
     end


 end

 function Game2ViewLayer:readAnimation(file, key, num, time,formatBit)
...
function Game2ViewLayer:initDiceView(  )
    ...
    local Button_goon = self._csbNode:getChildByName("Button_goon");
    Button_goon:setEnabled(false)
  	--荷官默认动作
   	--创建序列帧
 	local dealerAni = cc.Animate:create(cc.AnimationCache:getInstance():getAnimation("dealerDiceAnim"))         
   	...
   	--音效
    ExternalFun.playSoundEffect("watermargin_yaosaizi.mp3")
   	--左边
   	...
   	nodeLight:runAction(
   		cc.Sequence:create(
   			...
   			cc.CallFunc:create(function (  )
   				...
   				--等待音效
                ExternalFun.playSoundEffect("watermargin_xia.wav")
   	...
   	nodeTouzi:runAction(
   		cc.Sequence:create(
   			...
   			cc.CallFunc:create(function()
                ...
                local soundStr = string.format("watermargin_dian_%d.mp3",totalNum)
...
function Game2ViewLayer:initGame2Result()
	...
	if self._scene.m_pGame2Result.lScore > 0 then
		--赢的音效
        ExternalFun.playSoundEffect("watermargin_ying.mp3")
	else
		--输的音效
        ExternalFun.playSoundEffect("watermargin_shu.mp3")
	end
  	--默认动作
   	--创建序列帧
   	local animation --= cc.Animation:create()
			...
			node:runAction(
				cc.Sequence:create(
					cc.CallFunc:create(function (  )
                        ...
                        ExternalFun.playSoundEffect("watermargin_threegundong.wav")
					end)
					)
				)
		end
        ...
        cc.Sequence:create(
            ...
            cc.CallFunc:create(function (  )
                ...
                if 0 < #self._scene.m_pGame3Result then  --and 0 < #self._scene.m_pGame3Info    --#self._scene.m_pGame3Result > 0  then
                    ...
                    --移除页面
                    local gameview = self._scene._gameView
                    gameview._scene.m_bIsAuto = false 
                    self:backOneGame()
