...
function GameLayer:ctor( frameEngine,scene )
    ...
    self.m_pIconMessageBg:setContentSize(cc.size(700, 34))
    ...
    --self._gameFrame:SetDelaytime(10*60000) --设置10分钟没收到业务逻辑数据(玩家没操作)抛异常sser_TimeIsError(6)
    self:addAnimationEvent()  --监听加载完动画的事件
    self.m_bIsJieSuan = false
    self.m_lSceneScore = {}
    self.m_lSceneScore.score1 = 0
    self.m_lSceneScore.score2 = 0
end
...
function GameLayer:OnInitGameEngine()
    ...
    self.m_bMaxNum              = 1         --最大压分索引
    self.m_lBetScore            = {10,20,30,40,50,60,70,80}        --压分存储数组
    ...
    if MeItem and  MeItem.lScore < 90  then--and  MeItem.cbUserStatus > yl.US_FREE
        local seq = cc.Sequence:create(
            cc.CallFunc:create(function ( )
                ...
                showToast(cc.Director:getInstance():getRunningScene(), "提醒：游戏币不足", 2)
...
function GameLayer:onEventGameSceneFree(buffer)    --空闲
    ...
    --self.m_lTotalYafen = self.m_lBetScore[self.m_bYafenIndex]*self.m_lYafen*self.m_lYaxian
    --压分
    self._gameView.m_textYafen:setString(tostring(self.m_lBetScore[self.m_bYafenIndex]*self.m_lYafen))
    --总压分  Text_allyafen
    self._gameView.m_textAllyafen:setString(tostring(self.m_lTotalYafen))
    --重置玩家金币,得分
    self._gameView.m_textScore:setString(self:GetMeUserItem().lScore)
    self.m_lGetCoins = 0
...
function GameLayer:onGame1Start(dataBuffer) --游戏开始
    ...
    self.m_lSceneScore.score1 = self.m_lSceneScore.score1 +  cmd_table.lScore
    --进入小玛丽
    ...
    --设置按钮状态
    self._gameView:updateStartButtonState(2)    --停止
end
...
function GameLayer:onSubGame1End( dataBuffer )
    ...
    if self.m_lTotalYafen > self.m_lCoins + self.m_lGetCoins then
        --提示游戏币不足
        showToast(cc.Director:getInstance():getRunningScene(), "提醒：游戏币不足", 1)
        self.m_bIsAuto = false
        self._gameView:setAutoStart(false)
    else
        if self.m_bIsJieSuan == true then
            self.m_bIsAuto = false
            self._gameView:setAutoStart(false)
            self._gameView:onResultLayer(self.m_lSceneScore)
        end
...
function GameLayer:onGameStart()
    --2017.12.4 如果当前是小玛丽状态，禁止开始游戏
    if self.m_bEnterGame3 == true then
        return
    end
    ...
    if self.m_bIsAuto == true then  -- 自动开始
        ...
        elseif self:getGameMode() == GAME_STATE.GAME_STATE_WAITTING_GAME2  then
            ...
            self._gameView:stopAllActions()
            self._gameView:enableGame2Btn(false)    -- 游戏2按钮不可用
            self:sendGiveUpMsg()                    -- 发送消息
            self._gameView.m_textTips:setString("祝您好运！")
             if self.m_lTotalYafen > self.m_lCoins + self.m_lGetCoins then  -- 提示游戏币不足
                showToast(cc.Director:getInstance():getRunningScene(), "提醒：游戏币不足", 1)
                return
            end
            self:SendUserReady()
            
            self:sendReadyMsg()                     -- 发送准备消息
            self.m_cbGameStatus = g_var(cmd).SHZ_GAME_SCENE_FREE
            self:setGameMode(1)
        end
    end
    
    if self:getGameMode() == GAME_STATE.GAME_STATE_MOVING then  -- 开始
        self._gameView:game1End()
    elseif self._gameView.btnStartState == 3 then       -- 取分
        if self.m_lGetCoins and self.m_lGetCoins > 0 then
            self:sendGiveUpMsg()                        -- 发送放弃到比大小的消息
            self._gameView:enableGame2Btn(false)        -- 游戏2按钮不可用
            ExternalFun.playSoundEffect("watermargin_defen.wav")
            self._gameView.m_WinScore = self.m_lGetCoins
            self._gameView:takeWinScore()
            self.m_lGetCoins = 0
            self._gameView.m_textGetScore:setString(self.m_lGetCoins)
            self:setGameMode(0)
        end
    elseif self.m_bIsItemMove == false and (self:getGameMode() == GAME_STATE.GAME_STATE_WAITTING or self:getGameMode() == GAME_STATE.GAME_STATE_END) then
        if self.m_lTotalYafen > self.m_lCoins + self.m_lGetCoins then                       -- 发送放弃到比大小的消息
            showToast(cc.Director:getInstance():getRunningScene(), "提醒：游戏币不足", 1)   -- 提示游戏币不足
            return
        end
        self.m_bIsPlayed = true
        self._gameView:stopAllActions()
...
function GameLayer:onAutoStart( )
    ...
    if self.m_bIsAuto == true then
        ...
        if self.m_lGetCoins > 0 then                              
            --比倍开启
            self._gameView:enableGame2Btn(true)
            self._gameView:updateStartButtonState(3)      --收分
        end
    else
        self.m_bIsAuto = true
        if self.m_lTotalYafen > self.m_lCoins + self.m_lGetCoins then
...
function GameLayer:onAddMaxScore()
    ...
    self.m_lTotalYafen = self.m_lBetScore[self.m_bYafenIndex]*self.m_lYafen*self.m_lYaxian
    --压分
    self._gameView.m_textYafen:setString(self.m_lBetScore[self.m_bYafenIndex]*self.m_lYafen)
    --总压分
...
function GameLayer:onDoubleIn( )
    ...
    if self.m_lBaseYafen > self.m_lCoins then
        showToast(cc.Director:getInstance():getRunningScene(), "提醒：游戏币不足，请选择其他倍率", 1)
    else
        --发送消息
...
function GameLayer:onGame3Start( dataBuffer )
    ...
    self.m_lSceneScore.score2 = self.m_lSceneScore.score2 +  pGame3Info.lScore
...
function GameLayer:onGame3Result( dataBuffer )
    ...
    self.m_lSceneScore.score2 = self.m_lSceneScore.score2 +  pGame3Result.lScore
