...
--            end)))
--        end
        self._gameView:setClockPosition(viewid)
        self:OnEventGameClockInfo(viewid, id)
    end
end
...
function GameLayer:OnEventGameClockInfo(chair,time,clockid)
    ...
    if time < 5 then
        ExternalFun.playSoundEffect("zhajinhua_game_warn.mp3")
    end
...
function GameLayer:onEventGameScene(cbGameStatus,dataBuffer)
	...
	elseif cbGameStatus == cmd.GAME_STATUS_PLAY	then			--游戏状态
        ...
        for i = 1, cmd.GAME_PLAYER do
            self.m_lUserMaxScore[i] = dataBuffer:readscore(int64):getvalue()
        end
        ...
        local time = dataBuffer:readword()
        if not bCompareStatus then
            ...
            --设置时间
            self:SetGameClock(self.m_wCurrentUser, cmd.IDI_USER_ADD_SCORE, time)
        else
            if self:GetMeChairID() == self.m_wCurrentUser then
                --选择玩家比牌
                ...
                --设置时间
                self:SetGameClock(self.m_wCurrentUser, cmd.IDI_USER_COMPARE_CARD, time)
            else
                self._gameView:SetCompareCard(false)
                --设置时间
                self:SetGameClock(self.m_wCurrentUser, cmd.IDI_DISABLE, time)
            end
        end
	end
    -- 刷新房卡
...
function GameLayer:onSubGameStart(dataBuffer)
    ...
    --self._gameView.btChange:setEnabled(false)
    --self._gameView:setBtnEnabled(self._gameView.btChange,false)
    self.m_wBankerUser = dataBuffer:readword()
    self.m_wCurrentUser = dataBuffer:readword()

    --self._gameView:SetBanker(self:SwitchViewChairID(self.m_wBankerUser))
    self._gameView.m_lCellScore = self.m_lCellScore
    ...
    for i = 1, cmd.GAME_PLAYER  do
        ...
        print("m_cbPlayStatus:"..self.m_cbPlayStatus[i])
        if self.m_cbPlayStatus[i] == 1 then 
...
function GameLayer:onSubAddScore(dataBuffer)
    ...
    print("self.m_lCurrentTimes:"..self.m_lCurrentTimes)
    print("lCurrentTimes:"..lCurrentTimes)
    ...
    elseif self.m_lCurrentTimes == lCurrentTimes and lAddScoreCount > 0 then
        self._gameView:runFollowTimesAnimate(viewid)
    end
    ...
    if wCompareState == 0 then
        ...
        if self.m_cbPlayStatus[wAddScoreUser + 1] == 1 then
            if self.m_lCurrentTimes == lCurrentTimes then
                ExternalFun.playSoundEffect("BOY/zhajinhua_genzhu.mp3")
            else
                ExternalFun.playSoundEffect("BOY/zhajinhua_jiazhu.mp3")
            end
        end
        --设置计时器
        self:SetGameClock(self.m_wCurrentUser, cmd.IDI_USER_ADD_SCORE, cmd.TIME_USER_ADD_SCORE)
...
function GameLayer:onSubLookCard(dataBuffer)
    ...
    ExternalFun.playSoundEffect("zhajinhua_give_up.mp3")
    ExternalFun.playSoundEffect("BOY/zhajinhua_kanpai.mp3")
...
function GameLayer:onSubCompareCard(dataBuffer)
    ...
    ExternalFun.playSoundEffect("BOY/zhajinhua_bipai.mp3")
    ExternalFun.playSoundEffect("zhajinhua_compare_card.mp3")
end

function GameLayer:OnFlushCardFinish()

    local nodeCard = self._gameView.userCard[self:SwitchViewChairID(self.m_wLostUser)]
    if nodeCard ~=nil then
        for i = 1, 3 do
    ...
    if myChair == self.m_wWinnerUser then
        ...
        ExternalFun.playSoundEffect("zhajinhua_compare_win.mp3")
    elseif myChair == self.m_wLostUser then
        --self:PlaySound(cmd.RES.."sound_res/COMPARE_LOSE.wav")
        ExternalFun.playSoundEffect("zhajinhua_compare_lose.mp3")
    end
end
...
function GameLayer:onSubGiveUp(dataBuffer)
    ...
    ExternalFun.playSoundEffect("zhajinhua_give_up.mp3")
    ExternalFun.playSoundEffect("BOY/zhajinhua_fangqi.mp3")
end
...
function GameLayer:onSubGameEnd(dataBuffer)
        ...
        if lGameScore[i] ~= 0 then
            if lGameScore[i] > 0 then
                ...
                if viewid == cmd.MY_VIEWID then
                    print("播放胜利音乐")
                    ExternalFun.playSoundEffect("zhajinhua_game_win.mp3")
                else
                    ExternalFun.playSoundEffect("zhajinhua_game_lose.mp3")
                end
            else
                self._gameView:SetUserTableScore(viewid,lGameScore[i])
            end
            ...
            self._gameView.m_GameEndView:SetWinFlag(viewid, lGameScore[i], GameLogic:getCardType(cbCardData[i]))
    ...
    if endShow then
       self._gameView:ShowEndView(true) 
    end
    
    self._gameView.btReady:setVisible(true)
    --self._gameView.btChange:setEnabled(true)
    --self._gameView:setBtnEnabled(self._gameView.btChange,true)
    if not GlobalUserItem.bPrivateRoom then
        -- 设置倒计时
        self:SetGameClock(self:GetMeChairID(),cmd.IDI_START_GAME,cmd.TIME_START_GAME)
    end     
...
function GameLayer:addScore(index)
    ...
    if self.m_lCurrentTimes + scoreIndex > 10 then
        return
    end
...
function GameLayer:UpdataControl()
    ...
    if self.m_lUserMaxScore[MyChair] - self.m_lTableScore[MyChair] >= 
        ...
        if((self.m_lCurrentTimes*self.m_lCellScore) < self.m_lMaxCellScore) then
            ...
            if self.m_lCurrentTimes<=9 then
                self._gameView.btChip[1]:setEnabled(true)
                --self._gameView.btChip[1]:getVirtualRenderer():setState(0)
                --self._gameView.btChip[1]:setVisible(true)
            else
                self._gameView.btChip[1]:setEnabled(false)
                --self._gameView.btChip[1]:getVirtualRenderer():setState(1)
                --self._gameView.btChip[1]:setVisible(false)
            end

            if self.m_lCurrentTimes<=8 then
                self._gameView.btChip[2]:setEnabled(true)
--                self._gameView.btChip[2]:getVirtualRenderer():setState(0)
--                self._gameView.btChip[2]:setVisible(true)
            else
                self._gameView.btChip[2]:setEnabled(false)
--                self._gameView.btChip[2]:getVirtualRenderer():setState(1)
--                self._gameView.btChip[2]:setVisible(false)
            end

            if self.m_lCurrentTimes<=5 then
                self._gameView.btChip[3]:setEnabled(true)
--                self._gameView.btChip[3]:getVirtualRenderer():setState(0)
--                self._gameView.btChip[3]:setVisible(true)
            else
                self._gameView.btChip[3]:setEnabled(false)
--                self._gameView.btChip[3]:getVirtualRenderer():setState(1)
--                self._gameView.btChip[3]:setVisible(false)
            end
        else
