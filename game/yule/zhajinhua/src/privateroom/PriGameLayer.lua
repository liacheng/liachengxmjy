...
function PriGameLayer:onButtonClickedEvent( tag, sender )
    ...
    elseif BTN_INVITE == tag then
        ...
        PriRoom:getInstance():getPlazaScene():popTargetShare(clicktype,function(target, bMyFriend)
            ...
            local shareTxt = "炸金花约战 房间ID:" .. self.m_atlasRoomID:getString() .. " 局数:" .. PriRoom:getInstance().m_tabPriData.dwDrawCountLimit
            shareTxt = shareTxt .. " 炸金花游戏精彩刺激, 一起来玩吧! "
            local friendC = "炸金花约战 房间ID:" .. self.m_atlasRoomID:getString() .. " 局数:" .. PriRoom:getInstance().m_tabPriData.dwDrawCountLimit
            if bMyFriend then
            ...
            elseif nil ~= target then
                ...
                MultiPlatform:getInstance():shareToTarget(target, sharecall, "炸金花约战", shareTxt, url, "")
            end
        end)
