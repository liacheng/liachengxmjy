...
PreLoading.plistPath = {    "game_res/fishlk_fish_0.plist",
                            "game_res/fishlk_fish_1.plist",
                            "game_res/fishlk_fish_2.plist",
                            "game_res/fishlk_fish_3.plist",
                            "game_res/fishlkpy_game.plist",
					        ...
					        "game_res/fishlk_game_lockfish.plist",
                            "game_res/image.plist"           }

PreLoading.pngPath = {      "game_res/fishlk_fish_0.png",
                            "game_res/fishlk_fish_1.png",
                            "game_res/fishlk_fish_2.png",
                            "game_res/fishlk_fish_3.png",
                            "game_res/fishlkpy_game.png",
                            "game_res/whater.png",
					        "game_res/watch.png",
                            "game_res/wave.png",
					        ...
					        "game_res/bullet.png",
					        "game_res/fishlk_game_lockfish.png",
                            "game_res/image.png"            }
...
function PreLoading.loadTextures(scene)     -- GameViewLayer调用  scene = GameLayer
	...
	local totalSource = 15                  -- 总共需加载的资源数量
...
function PreLoading.readAniams()
    ...
    local fishFrameMoveNum = { 12, 16, 24, 24, 24, 25, 60, 20, 24, 16, 24, 12, 24, 20, 24, 24, 24, 9, 9, 20, 15, 15, 8, 1}
	local fishFrameDeadNum = { 5,   3,  3, 10,  3,  6,  3,  6,  4,  7,  4,  4,  4,  3,  6,  6,  4, 3, 4, 20,  9, 15, 8, 2}
	...
	for index = 0, PreLoading.FishTypeNum-1 do   --鱼游动动画
   	...
   	PreLoading.readAnimation("lkpy_gold_coin_","FishCoin",7,0.08,1)
    ...
    PreLoading.readAnimation("lkpy_silver_coin_", "SilverAnim", 12, 0.05,1);
    PreLoading.readAnimation("lkpy_gold_coin_", "GoldAnim", 7, 0.08,1);
...
function PreLoading.GameLoadingView()                                   -- 创建读取界面(读取界面图层, 背景图, 进度条背景, 进度条, 鱼)
    ...
    fish1:setPosition(cc.p(750 + xoffset, 594))
    fish1:runAction(
        cc.RepeatForever:create(
            cc.Sequence:create(
                cc.Spawn:create(cc.RotateTo:create(1, 5), cc.MoveTo:create(1, cc.p(750 + xoffset, 604))), 
                cc.Spawn:create(cc.RotateTo:create(1,-5), cc.MoveTo:create(1, cc.p(750 + xoffset, 584)))
            )
        )
    )
    ...
    fish2:setPosition(cc.p(1120 + xoffset, 270))
    fish2:runAction(
        cc.RepeatForever:create(
            cc.Sequence:create(
                cc.Spawn:create(cc.RotateTo:create(1,-5), cc.MoveTo:create(1, cc.p(1120 + xoffset, 260))), 
                cc.Spawn:create(cc.RotateTo:create(1, 5), cc.MoveTo:create(1, cc.p(1120 + xoffset, 280)))
            )
        )
    )
    layer:addChild(fish2)

    local fish3 = display.newSprite("loading/loading_icon_2.png")
    fish3:setPosition(cc.p(194 + xoffset, 334))
    fish3:setOpacity(0)
    fish3:runAction(
        cc.Spawn:create(
            cc.FadeIn:create(1), 
            cc.MoveTo:create(1, cc.p(339 + xoffset, 416))
        )
    )
