...
function Gold:showGold(curScore)
    ...
    if #self.m_goldList >= 3 then
        ...
        local gold = cc.Sprite:create("game_res/lkpy_icon_golds.png", cc.rect(0, 0, 38, 7));
        ...
        local goldNum = cc.LabelAtlas:create(num,"game_res/lkpy_num_small.png",18,25, string.byte("*"))
...
--拓展显示金币
function Gold:showGoldEx(curScore)
    local num = string.format("%d",curScore)
    local goldNum = cc.LabelAtlas:create(num,"game_res/lkpy_num_bigfish.png",54,75,string.byte("/"))
    goldNum:setScale(0.7)
    goldNum:setAnchorPoint(cc.p(0.5,0))
    goldNum:setPosition(cc.p(-43+280,-54))
    self:addChild(goldNum)

    local coin = display.newSprite("#lkpy_icon_coin.png")
    coin:setAnchorPoint(cc.p(0.5,0.5))
    coin:setScale(2)
    coin:setPosition(cc.p(-30,35))
    goldNum:addChild(coin)

    goldNum:runAction(cc.Sequence:create(cc.DelayTime:create(0.1),cc.MoveTo:create(0.3, cc.p(-43+280,-10)), cc.RemoveSelf:create()))
