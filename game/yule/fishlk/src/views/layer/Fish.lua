...
--        end
	...
	if nil == self.m_scheduleSceneFish then     -- 定时器
		self.m_scheduleSceneFish = scheduler:scheduleScriptFunc(updateFish, 0.05/self.m_fDelay, false)
	end
end
...
function Fish:slowDown()
    local function updateFish(dt)
        ...
        if self.m_pathIndex >  #self.m_data.init_pos[1] - 2 then
            if nil ~= self.m_scheduleSceneFish then
                scheduler:unscheduleScriptEntry(self.m_scheduleSceneFish)
                self.m_scheduleSceneFish = nil
	       end
           ...
           local distance = self:CalcDistance(self.m_data.init_pos[1][#self.m_data.init_pos[1]].x,self.m_data.init_pos[1][#self.m_data.init_pos[1]].y,self.m_data.init_pos[1][#self.m_data.init_pos[1]-2].x,self.m_data.init_pos[1][#self.m_data.init_pos[1]-2].y)
           local swimTime = distance/100
           local angle = self._dataModel:getAngleByTwoPoint(cc.p(self.m_data.init_pos[1][#self.m_data.init_pos[1]].x,self.m_data.init_pos[1][#self.m_data.init_pos[1]].y),self.m_oldPoint)
           self:setRotation(angle)
           local bt = cc.MoveTo:create(swimTime,self:convertPoint(cc.p(self.m_data.init_pos[1][#self.m_data.init_pos[1]].x,self.m_data.init_pos[1][#self.m_data.init_pos[1]].y)))
           local action = cc.Sequence:create(bt,cc.CallFunc:create(removeCallBack))
           action:setTag(FISHTAG.TAG_ACTION)
           self:runAction(action)
           return
        end
        self:setPosition(cc.p(self.m_data.init_pos[1][self.m_pathIndex].x,self.m_data.init_pos[1][self.m_pathIndex].y))
        local angle = self._dataModel:getAngleByTwoPoint(cc.p(self.m_data.init_pos[1][self.m_pathIndex].x,self.m_data.init_pos[1][self.m_pathIndex].y),self.m_oldPoint)
        self:setRotation(angle)
        self.m_oldPoint = cc.p(self.m_data.init_pos[1][self.m_pathIndex].x,self.m_data.init_pos[1][self.m_pathIndex].y)
        self.m_pathIndex = self.m_pathIndex + 1
    end
    ...
    self.m_fDelay = 0.7
    if self:getActionByTag(FISHTAG.TAG_ACTION) then
        self:setActionDelayByTag(FISHTAG.TAG_ACTION, self.m_fDelay)
    end
    ...
    self:stopActionByTag(FISHTAG.TAG_ACTION+1)
    ...
    action:setTag(FISHTAG.TAG_ACTION+1)
    self:runAction(action)
end
