...
function Cannon:initWithUser(userItem)
	...
	self.m_fort = display.newSprite("#lkpy_gun_1_1.png")
    ...
    --底座
	self.m_gunPlatformButtom = display.newSprite("#lkpy_gunPlatformButtom1_1.png")
    ...
    self.m_labelMutiple = cc.LabelAtlas:create("1","game_res/lkpy_num_small.png",18,25,string.byte("*")) --倍数
    ...
    if self.m_ChairID == self.m_nChairID then
        --加减炮按钮 
        self.btnAdd = ccui.Button:create("lkpy_btn_addgun.png","lkpy_btn_addgun.png","lkpy_btn_addgun.png",UI_TEX_TYPE_PLIST)
        ...
        self.btnMinus = ccui.Button:create("lkpy_btn_deletegun.png","lkpy_btn_deletegun.png","lkpy_btn_deletegun.png",UI_TEX_TYPE_PLIST)
...
function Cannon:setMultiple(multiple,chairId)
    ...
    if self.m_SpecialCannon == true then
        frame = cc.SpriteFrameCache:getInstance():getSpriteFrame("lkpy_gun_1_2.png")
        gunPlatform = cc.SpriteFrameCache:getInstance():getSpriteFrame("lkpy_gunPlatformButtom2_1.png")
        if multiple >= 100 and multiple < 1000 then
            frame = cc.SpriteFrameCache:getInstance():getSpriteFrame("lkpy_gun_2_2.png")
            gunPlatform = cc.SpriteFrameCache:getInstance():getSpriteFrame("lkpy_gunPlatformButtom2_2.png")
        elseif multiple >= 1000 and multiple < 5000 then
            frame = cc.SpriteFrameCache:getInstance():getSpriteFrame("lkpy_gun_3_2.png")
            gunPlatform = cc.SpriteFrameCache:getInstance():getSpriteFrame("lkpy_gunPlatformButtom2_3.png")
        elseif multiple >= 5000 then
            frame = cc.SpriteFrameCache:getInstance():getSpriteFrame("lkpy_gun_4_2.png")
            gunPlatform = cc.SpriteFrameCache:getInstance():getSpriteFrame("lkpy_gunPlatformButtom2_4.png")
        end
    elseif self.m_SpecialCannon == false then 
        frame = cc.SpriteFrameCache:getInstance():getSpriteFrame("lkpy_gun_1_1.png")
        gunPlatform = cc.SpriteFrameCache:getInstance():getSpriteFrame("lkpy_gunPlatformButtom1_1.png")
        if multiple >= 100 and multiple < 1000 then
            frame = cc.SpriteFrameCache:getInstance():getSpriteFrame("lkpy_gun_2_1.png")
            gunPlatform = cc.SpriteFrameCache:getInstance():getSpriteFrame("lkpy_gunPlatformButtom1_2.png")
        elseif multiple >= 1000 and multiple < 5000 then
            frame = cc.SpriteFrameCache:getInstance():getSpriteFrame("lkpy_gun_3_1.png")
            gunPlatform = cc.SpriteFrameCache:getInstance():getSpriteFrame("lkpy_gunPlatformButtom1_3.png")
        elseif multiple >= 5000 then
            frame = cc.SpriteFrameCache:getInstance():getSpriteFrame("lkpy_gun_4_1.png")
            gunPlatform = cc.SpriteFrameCache:getInstance():getSpriteFrame("lkpy_gunPlatformButtom1_4.png")
        end
    end
...
function Cannon:setMyMultiple(multiple)
    ...
    local mutipleStr = string.format("%d", multiple)
    self.m_labelMutiple:setString(mutipleStr)
    local frame = cc.SpriteFrameCache:getInstance():getSpriteFrame("lkpy_gun_1_1.png")
    local gunPlatform = cc.SpriteFrameCache:getInstance():getSpriteFrame("lkpy_gunPlatformButtom1_1.png")
    if multiple >= 100 and multiple < 1000 then
        frame = cc.SpriteFrameCache:getInstance():getSpriteFrame("lkpy_gun_2_1.png")
        gunPlatform = cc.SpriteFrameCache:getInstance():getSpriteFrame("lkpy_gunPlatformButtom1_2.png")
    elseif multiple >= 1000 and multiple < 5000 then
...
function Cannon:othershoot(firedata)  -- 其他玩家开火
    ...
    local fish = nil
    if  firedata.lock_fishid > 0 then
        fish = self._dataModel.m_fishList[firedata.lock_fishid]
    elseif firedata.lock_fishid  == -1 then 
        local fishBig =  self._dataModel:selectMaxFish()
        fish = self._dataModel.m_fishList[fishBig]
    end

    if fish ~= nil and firedata.lock_fishid ~= g_var(cmd).INT_MAX then
        if self._dataModel._exchangeSceneing == true then
            return 
        end
        local fishData = fish.m_data
    ...
    if firedata.lock_fishid == g_var(cmd).INT_MAX or firedata.lock_fishid == 0 or fish == nil then
...
function Cannon:setLockFishLogo(chairId,fishKind)
    local frameName = string.format("lkpy_lock_flag_%d.png", fishKind+1)
    local frame = cc.SpriteFrameCache:getInstance():getSpriteFrame(frameName)

    local switchfish = cc.SpriteFrameCache:getInstance():getSpriteFrame("lkpy_switch_fish.png")
    ...
    if nil ~= frame then
        ...
        local sp1 = self:getChildByTag(TagEnum.Tag_switch_fish + chairId)
        if nil == sp then
            ...
            btnswitchfish:setTag(TagEnum.Tag_btn_switch + chairId)
            ...
            sp1:setTag(TagEnum.Tag_switch_fish + chairId)
            sp1:setName("lkpy_switch_fish.png")
            sp1:setPosition(pos1.x,pos1.y)
            self:addChild(sp1)
            sp1:runAction(cc.RepeatForever:create(CirCleBy:create(1.0,cc.p(pos1.x,pos1.y),10)))

            local btnswitchfish1 = ccui.Button:create("lkpy_switch_fish.png","lkpy_switch_fish.png","lkpy_switch_fish.png",UI_TEX_TYPE_PLIST)
            btnswitchfish1:setPosition(pos1.x,pos1.y)
            btnswitchfish1:setAnchorPoint(0.5,0.5)
            btnswitchfish1:setTag(TagEnum.Tag_btn_switch1 + chairId)
...
function Cannon:ButtonEvent( sender , eventType)
    ...
    if tag == TagEnum.Tag_btn_switch + self.m_nChairID  or tag == TagEnum.Tag_btn_switch1 + self.m_nChairID then 
...
function Cannon:productBullet( isSelf,fishIndex, netColor,bundleChairId,bullet_mulriple,bulletId,android_chairid,bulletSpeedIndex)  -- 制造子弹
    ...
    local gun_spark = display.newSprite("#lkpy_iocn_fire.png")
...
--        return
		self.m_fort:setSpriteFrame(cc.SpriteFrameCache:getInstance():getSpriteFrame("lkpy_gun_1_1.png"))
        self.m_gunPlatformButtom:setSpriteFrame(cc.SpriteFrameCache:getInstance():getSpriteFrame("lkpy_gunPlatformButtom1_1.png"))
	...
	elseif cannontype == g_var(cmd).CannonType.Laser_Cannon then
		...
		ExternalFun.playSoundEffect(g_var(cmd).SmallBegin)
	...
	elseif cannontype == g_var(cmd).CannonType.Normal_Cannon then
		self.m_Type = g_var(cmd).CannonType.Normal_Cannon
		self.m_fort:setSpriteFrame(cc.SpriteFrameCache:getInstance():getSpriteFrame("lkpy_gun_1_1.png"))
        self.m_gunPlatformButtom:setSpriteFrame(cc.SpriteFrameCache:getInstance():getSpriteFrame("lkpy_gunPlatformButtom1_1.png.png"))
		self:removeTypeTag()
	end
end
...
--        isAndroid = self.frameEngine:getTableUserItem(self.m_nTableID, fire.chair_id)
--    end

    if isAndroid then
        local randomfiretimes = math.random(0,5)
        local randomangle = 0
        if randomfiretimes == 3  then
           randomangle = math.random(-2,2)
        end
        
        if self._dataModel.m_reversal then
            if fire.chair_id  == 0 then
--                angle = -math.abs(fire.angle) - 5
                angle = 180*(fire.angle + randomangle)/6.28-90
            end
...
--                angle = 180 + math.abs(fire.angle) + 5
                angle = 180*(fire.angle + randomangle)/6.28
            end
            if fire.chair_id  == 4 then
--                angle = 180 + math.abs(fire.angle) + 5
                angle = 180*(fire.angle + randomangle)/6.28
            end
...
--                angle = 180 + math.abs(fire.angle) + 5
            ...
            if fire.chair_id  == 3 then
--                angle = math.abs(fire.angle) + 5
                angle = 180*(fire.angle + randomangle)/6.28 + 180
            end
            if fire.chair_id  == 4 then
--                angle = math.abs(fire.angle) + 5
                angle = 180*(fire.angle + randomangle)/6.28 + 180
...
--                angle = -math.abs(fire.angle) - 5
    ...
    if fire.lock_fishid == -1 then
        local fishBig =  self._dataModel:selectMaxFish();
        fish1 = self._dataModel.m_fishList[fishBig]
        if fish1 ~= nil then
		    ...
		    angle = self._dataModel:getAngleByTwoPoint(pos, self.m_cannonPoint)
            self:setLockFishLine(fire.chair_id,fishBig)
        end
    ...
    if self.frameEngine:GetMeUserItem() ~= nil then
        if fish1 ~= nil then
            self:productBullet(false, fish1.nFishKey, cc.WHITE,fire.chair_id,fire.bullet_mulriple,fire.bullet_id,fire.android_chairid,bulletSpeedIndex)
        else
...
function Cannon:removeLockTag(chairId)
    if self:getChildByTag(TagEnum.Tag_lock + chairId) then
        ...
        self:removeChildByTag(TagEnum.Tag_switch_fish + chairId)
        self:removeChildByTag(TagEnum.Tag_btn_switch + chairId)
        self:removeChildByTag(TagEnum.Tag_btn_switch1 + chairId)
    end
