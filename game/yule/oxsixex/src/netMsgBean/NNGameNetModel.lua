...
function NNGameNetModel:readSubFreeStatues(dataBuffer)
    ...
    t_data.szGameRoomName      = dataBuffer:readstring(32)                 --房间名称
    ...
    t_data.lBonus = dataBuffer:readscore(int64):getvalue()                 --彩金金额
    t_data.bIsSuportBonus = dataBuffer:readbool()                          -- 彩金版
    return t_data
end
...
function NNGameNetModel:readSubPlayingStatues(dataBuffer)
    ...
    t_data.lBonus = dataBuffer:readscore(int64):getvalue()                 --彩金金额
    t_data.bIsSuportBonus = dataBuffer:readbool()                          -- 彩金版
    t_data.cbTimeLeave = dataBuffer:readbyte()            
...
--用户中奖信息
function NNGameNetModel:readSubWinUser(dataBuffer)
    local len = dataBuffer:getlen()
    local recordcount = math.floor(len / cmd.WINRRCORD_LENGTH);
    if (len - recordcount * cmd.WINRRCORD_LENGTH) ~= 0 then
    ...
    for i=1,recordcount do
        local nickName = dataBuffer:readstring(32)
        if nickName == "" then
             break 
        end
        game_record[i] = {}
        game_record[i].szNickName = nickName
        game_record[i].time = dataBuffer:readdword()
        game_record[i].winGold = dataBuffer:readscore(int64):getvalue()
    end
    return game_record
end
...
function NNGameNetModel:readSubGameEnd(dataBuffer)
    ...
    for i=1,cmd.GAME_PLAYER do 
        t_data.lSubBobus[i] = dataBuffer:readscore(int64):getvalue() --奖池得分
    end
    
    t_data.lBonus = dataBuffer:readscore(int64):getvalue() --奖池分数
    print("奖池得分：",t_data.lBobus )
