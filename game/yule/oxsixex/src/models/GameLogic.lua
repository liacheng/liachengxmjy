...
GameLogic.OX_FIVE_KING          = 22                                --五花牛牌
GameLogic.OX_FIVE_KING_SOFT     = 23                                --软五花牛牌
GameLogic.OX_FIVE_KING_HAND     = 24                                --硬五花牛牌
GameLogic.OX_THREE_BAR           = 25                               --三条牛牌
GameLogic.OX_VALUE_GOURD        = 26                                --葫芦牛牌
GameLogic.OX_FOUR_BAR           = 27                                --葫芦牛牌
...
--获取类型
function GameLogic.GetCardType(cbCardData,cbCardCount,bIsSuportBonus) 
	...
	if cbKingCount == 2 then
        ...
        --特殊牌型
		local cbTenCount = 0;
		local cbTenUpCount = 0;
		for i=1,cbCardCount do
            local cardValues = GameLogic.GetCardValue(cbCardData[i])
			if cardValues >= 10 then
                if cardValues == 10 then
                    cbTenCount = cbTenCount + 1
                end
				cbTenUpCount = cbTenUpCount + 1
			end
		end

        if cbTenUpCount == GameLogic.MAX_COUNT and  cbTenCount >= 1 and bIsSuportBonus then
            return GameLogic.OX_FIVE_KING_SOFT
        end
	...
	elseif cbKingCount == 0 then
        if bIsSuportBonus then
		    ...
		    for i=1,cbCardCount do
                local cardValues = GameLogic.GetCardValue(cbCardData[i])
			    if cardValues > 10 then
                    if cardValues == 11 then
                        cbJCount = cbJCount + 1
                    elseif cardValues == 12 then
                        cbQCount = cbQCount + 1
                    elseif cardValues == 13 then
                        cbKCount = cbKCount + 1
                    end
				    cbJQKCount = cbJQKCount + 1
			    elseif cardValues == 10 then
				    cbTenCount = cbTenCount + 1
			    end
		    end
		    if cbJCount==(GameLogic.MAX_COUNT -1) or cbQCount == (GameLogic.MAX_COUNT -1) or cbKCount == (GameLogic.MAX_COUNT -1) then
                return GameLogic.OX_FOUR_BAR
            elseif (cbJCount==(GameLogic.MAX_COUNT -2) or cbQCount == (GameLogic.MAX_COUNT -2) or cbKCount == (GameLogic.MAX_COUNT -2))
                    and (cbJCount==(GameLogic.MAX_COUNT -3) or cbQCount == (GameLogic.MAX_COUNT -3) or cbKCount == (GameLogic.MAX_COUNT -3)) then
                return GameLogic.OX_VALUE_GOURD
            elseif cbJCount==(GameLogic.MAX_COUNT -2) or cbQCount == (GameLogic.MAX_COUNT -2) or cbKCount == (GameLogic.MAX_COUNT -2) then
                return GameLogic.OX_THREE_BAR
            elseif cbJQKCount == GameLogic.MAX_COUNT then
                return GameLogic.OX_FIVE_KING_HAND
            elseif cbTenCount>=1  and cbJQKCount == (GameLogic.MAX_COUNT - cbTenCount) then
                return GameLogic.OX_FIVE_KING_SOFT
            end
        else
            if cbJQKCount == 5 then
			    return OX_FIVE_KING;
            end
        end
...
--获取倍数
function GameLogic.GetTimes(cbCardData, cbCardCount,bIsSuportBonus)
	if cbCardCount ~= GameLogic.MAX_COUNT then
        return 0;
    end

	local bTimes = GameLogic.GetCardType(cbCardData, GameLogic.MAX_COUNT,bIsSuportBonus);
...
function GameLogic.getOxValue(cbValue)
    ...
    elseif cbValue == GameLogic.OX_VALUE_10 or cbValue == GameLogic.OX_VALUE_BIG_10 
        or cbValue == GameLogic.OX_FIVE_KING_SOFT  or cbValue == GameLogic.OX_FIVE_KING_HAND
        or cbValue == GameLogic.OX_THREE_BAR  or cbValue == GameLogic.OX_VALUE_GOURD
        or cbValue == GameLogic.OX_FOUR_BAR  or cbValue == GameLogic.OX_FIVE_KING then
        iValue = 10
    else
        iValue = 0xFF
    end
