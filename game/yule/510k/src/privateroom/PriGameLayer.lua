...
function PriGameLayer:ctor( gameLayer )
    ...
    --约战模式 self.m_bIsYueZhan
    self._gameLayer.m_bIsYueZhan = true
    ...
    self.img_rule:setVisible(false)
    ...
    --4,6王
    self.text_rule = image_bg:getChildByName("text_rule")
    if self._gameLayer.m_ModelWang == 1 then
        self.text_rule:setString("4王")
    else
       self.text_rule:setString("6王")
    end
...
function PriGameLayer:onPriGameEnd( cmd_table )
    ...
    for i = 1, chairCount do
        ...
        -- 成绩
        local strScore = ""
        if self._gameLayer.m_bIsYueZhan == false then
            strscore = score .. ""
            if score > 0 then
                    strscore = "+" .. score
            end
        else
           strScore = ""..self._gameLayer._gameView.m_lCollectScore[i]
           if self._gameLayer._gameView.m_historyScore[viewId].lCollectScore > 0 then
               strScore = "+"..self._gameLayer._gameView.m_lCollectScore[i]
           end
        end
        ...
        -- 游戏总结算输赢图标判断
        local totalscore = self._gameLayer._gameView.m_lCollectScore[i]
