...
function ResultLayer:ctor( parent,rs)
    ...
    if self.m_parent._scene.m_bIsYueZhan == true then
       self:initYueZhan(rs)
    else
       self:init(rs)
    end
...
function ResultLayer:init(rs)
    ...
    for i = 1,cmd.PLAYER_COUNT do
        ...
        local curScore = self.m_spBg:getChildByName(string.format("curScore_%d", viewId))           --当前分数
        curScore:setString(string.format("%d分",rs.lGameScore[1][i]))                              -- rs.PlayerScore[1][i]

        local curCellTimes = self.m_spBg:getChildByName(string.format("curCellTimes_%d", viewId))
        curCellTimes:setString(string.format("%d",1))                                               --rs.cbUserTimes[1][i]  
...
--约战模式下的初始化
function ResultLayer:initYueZhan(rs)
    --加载csb资源
    local csbNode = ExternalFun.loadCSB("game_res/ResultLayer.csb", self)
    ...
    for i = 1,cmd.PLAYER_COUNT do
        ...
        --基本得分
        local baseScore = self.m_spBg:getChildByName(string.format("curScore_%d", viewId))           
        baseScore:setString(string.format("%d分",rs.lBaseScore[1][i]))                              
        --彩数
        local curCaiShu = self.m_spBg:getChildByName(string.format("curCellTimes_%d", viewId))
        curCaiShu:setString(string.format("%d",rs.cbCaiShu[1][i]))                                               
        --本局得分
        local curResult = self.m_spBg:getChildByName(string.format("curResult_%d", viewId))
        local score = rs.lRoundScore[1][i]
        local img_result = self.m_spBg:getChildByName(string.format("img_result_%d", viewId))
...
function ResultLayer:showGameResult(rs)
    ...
    if self.m_parent._scene.m_bIsYueZhan == true then
        self:showResultYueZhan(rs)
    else
        self:showResultCoin(rs)
    end
end
--显示约战游戏结算
function ResultLayer:showResultYueZhan(rs)
    for i = 1,cmd.PLAYER_COUNT do
        local viewId = self.m_parent._scene:SwitchViewChairID(i - 1)
        local userItem = self.m_parent.m_historyUseItem[viewId]
        if nil == userItem then
...
--显示金币场游戏结算
function ResultLayer:showResultCoin(rs)
    for i = 1,cmd.PLAYER_COUNT do
        local viewId = self.m_parent._scene:SwitchViewChairID(i - 1)
        local userItem = self.m_parent.m_historyUseItem[viewId]
        if nil == userItem then
        ...
        curScore:setString(string.format("%d分",rs.PlayerScore[1][i]))

        local curCellTimes = self.m_spBg:getChildByName(string.format("curCellTimes_%d", viewId))
        curCellTimes:setString(string.format("%d", 0))          --rs.cbUserTimes[1][i]  
