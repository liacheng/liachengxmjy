...
--纹理宽高
local CARD_WIDTH = 133      --大牌的宽158   133
local CARD_HEIGHT = 174     --大牌的高208   174
local BACK_Z_ORDER = 2      --背牌的zorder
...
--创建卡牌
function CardSprite:createCard( cbCardData, flag,tagParam )
    flag = flag or 0
	local sp = CardSprite.new(flag);
...
--更新牌的唯一标志
function CardSprite:updateFlag(flag)
    self.flag = flag
    self:setTag(flag);
end

function CardSprite:getFlag()
   return self.flag
end
--更新纹理资源
