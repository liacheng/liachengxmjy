...
--设置名次信息
function CardsNode:setWinOrderInfo(wWinOrder)
    if wWinOrder <= 0 then 
        return
    end
    ...
    if wWinOrder == 1 then
        strWinOrder = "一"
    elseif wWinOrder == 2 then
        strWinOrder = "二"
    elseif wWinOrder == 3 then
        strWinOrder = "三"
    end
    if self.m_winOrderInfoText == nil then
       	self.m_winOrderInfoText = cc.Label:createWithTTF("第"..strWinOrder.."名" , appdf.FONT_FILE, 36)
        ...
        self.m_winOrderInfoText:setString("第"..strWinOrder.."名")
    end

    self.m_winOrderInfoText:setVisible(true)
end

--清空名次信息，在游戏结束时
function CardsNode:emptyWinOrderInfo()
    if self.m_winOrderInfoText ~= nil then
       self.m_winOrderInfoText:setVisible(false)
    end
end
...
function CardsNode:onTouchBegan(touch, event)
	...
	if nil ~= self.m_beginSelectCard 
		...
		self.m_mapSelectedCards[self.m_beginSelectCard:getFlag()] = self.m_beginSelectCard
	end
		...
		if nil ~= self.m_endSelectCard 
			...
			if false == self.m_endSelectCard:getCardDragSelect() then
				...
				if nil ~= self.m_beginSelectCard 
					...
					self.m_mapDragSelectCards[self.m_endSelectCard:getFlag()] = self.m_endSelectCard
				end
			end
		end
...
function CardsNode:onTouchEnded(touch, event)
	if true == self.m_bTouched then
		...
		if false == self.m_bDragCard then
			if nil ~= self.m_endSelectCard and nil ~= self.m_endSelectCard.getCardData then
				self.m_endSelectCard:setCardDragSelect(true)

                local flag = self.m_endSelectCard:getFlag()

				if nil ~= self.m_beginSelectCard and nil ~= self.m_beginSelectCard.getCardData 
                    and self.m_beginSelectCard:getCardData() ~= self.m_endSelectCard:getCardData() then

					self.m_mapSelectedCards[self.m_endSelectCard:getFlag()] = self.m_endSelectCard
				end
			end
			--选牌音效
...
function CardsNode:updateCardsNode( cards, isShowCard, bAnimation, pCallBack)
	...
	----------创建扑克m_pSelectedListener,就是对应的玩家-------
	...
	for i = 1, m_cardCount do
		local tmpSp = CardSprite:createCard(m_cardsData[i], i)
	...
	if ((cmd.RIGHT_VIEWID == self.m_nViewId) 
        ...
        --moveTo和backTo是洗牌动画,arrangeAllCards是发牌的动画
		for i = 1, m_cardCount do
...
function CardsNode:addCards( addCards, handCards )
    ...
    self:updateAllCardsFlag()

	for i = 1, #addCards do
		local tmpSp = CardSprite:createCard(addCards[i], i)
		tmpSp:setPosition(centerPos)
		tmpSp:setDispatched(false)
		tmpSp:showCardBack(true)
		self.m_cardsHolder:addChild(tmpSp)
		self.m_vecCard[self.m_cardsCount + i] = tmpSp
	end
...
function CardsNode:outCard( cards, bNoSubCount)
	...
	local bOutOk = false
    local mapOutCards = self:getMapOutCards(cards)
	...
	if 0 ~= handCount and haveCardData then
		...
		for k,v in pairs(mapOutCards) do
			local tag = k
...
-- 从手牌中找到 出的牌 对应的下标
function CardsNode:getMapOutCards(outCards)
   local mapOutCards = {}
   local cardsData = GameLogic:deepCopy(self.m_cardsData)
   for k,v in pairs(outCards) do
        local idx = nil
        for k1,v1 in pairs(cardsData) do            
            if v == v1 then
                idx = k1
            end
        end
...
function CardsNode:showCards()
	for k,v in pairs(self.m_vecCard) do
...
function CardsNode:showLeftCards( cards )
	...
	if #cards <= self.m_cardsCount then
		for i = 1, self.m_cardsCount do
			...
			if nil ~= tmp and nil ~= cbCardData then
...
--将提示的选牌找出放入到
function CardsNode:tipCardsIntoSelect(cards)
   local vecCardSprite = GameLogic:deepCopy(self.m_vecCard)
   for i = 1, #cards do
		local tmp = nil
        for k,v in pairs(vecCardSprite) do
           local value = v:getCardData()
		...
		if nil ~= tmp then
			...
			self.m_mapSelectedCards[tmp:getFlag()] = tmp
		end
    end
end
--根据牌值获取对应的卡牌
function CardsNode:getCardSpriteByCardData(cardData)
   for k,v in pairs(self.m_vecCard) do
       local value = v:getCardData()
       if value == cardData then
          return v
       end
   end
end
		...
		if cmd.MY_VIEWID == self.m_nViewId then
			...
			for i = 1, count do
				...
				if nil ~= tmp then
                    ...
                    --判断是否是伙牌,加上伙 图
                    self:showFlagForPartnerCard(tmp)
...
function CardsNode:reSortCards()
	...
	if cmd.MY_VIEWID == self.m_nViewId then
		...
		for i = 1, count do
			local cardData = cards[i]
			cardData = (cardData ~= 0) and cardData or i
			local tmp = self.m_vecCard[cardData]
			if nil ~= tmp then
				tmp:setLocalZOrder(i)
				tmp:setDispatched(true)
				tmp:showSelectEffect(false)
...
--将伙伴牌标记出
function CardsNode:showFlagForPartnerCard(tmp)
    --判断是否是伙牌,加上伙 图        
    local cardData = tmp:getCardData()
    if GameLogic.m_cbCardDataPartner == cardData and cardData ~= 0 then
        local height = tmp:getBoundingBox().height
        local imgHuoBan = ccui.ImageView:create("game_res/play_huoban.png")
	    imgHuoBan:move(10,height/2)
	    imgHuoBan:addTo(tmp)
	    imgHuoBan:setName("imgHuoBan")
    end 
end
--更新所有卡牌的flag和tag
function CardsNode:updateAllCardsFlag()
    local vecCard = {}
    local i = 0
    for k,v in pairs(self.m_vecCard) do
        i = i + 1
...
function CardsNode:dragCards( vecCard )
	...
	for k,v in pairs(vecCard) do
		...
		if not v:getCardShoot() then
            ...
            self.m_mapSelectedCards[v:getFlag()] = v
        else
            ...
            self.m_mapSelectedCards[v:getFlag()] = nil
		end
	end
...
function CardsNode:setCardsBlack(vecCard)
	for ke,va in pairs (vecCard) do
		...
		for k,v in pairs (self.m_vecCard) do
			local pCardSprite = self.m_vecCard[k]
