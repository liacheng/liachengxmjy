...
local CARD_WIDTH_SMALL = 40     --小牌的宽96 ,40
local CARD_HEIGHT_SMALL = 50    --小牌的高127 ,50
local CARD_DISTANCE_UNIT = 8    --小牌的间距12, 8
local CARD_SCALE = 2.5          --小牌的倍数
...
function GameViewLayer:paramInit()
    ...
    self.m_bMyOutCards = false  --是否是我出牌
    --解散
...
function GameViewLayer:loadResource()
    ...
    --播放背景音乐
    ExternalFun.setBackgroundAudio("background.wav")

    local rootLayer, csbNode = ExternalFun.loadRootCSB("game_res/GameLayer.csb", self)
    self.m_rootLayer = rootLayer
    self.m_csbNode = csbNode
    self.m_csbNode:setAnchorPoint(cc.p(0.5,0.5))
    self.m_csbNode:setPosition(cc.p(667,375))               --cc.p(667,375)
    ...
    for i = 1, 4 do
        ...
        --身份显示
        local flag = head_bg:getChildByName("imgFlag")
        flag:setVisible(false)
        flag:setLocalZOrder(TAG.Tag_Head + 5)
        --彩数
        local caiShuImg = head_bg:getChildByName("imgCaiShu")
        local caiShuText = caiShuImg:getChildByName("textCaiShu")
        caiShuText:setVisible(true)
        caiShuText:setLocalZOrder(TAG.Tag_Head + 5)
        ...
        local caiShuScore = self._scene:getCaiShuScore(i)
        local goldImg = head_bg:getChildByName("gold_img")
        local text_curScore = head_bg:getChildByName("text_curScore")
        text_curScore:setString("当前得分:"..caiShuScore)
        --约战和非约战
        if self._scene.m_bIsYueZhan == true then
            text_curScore:setVisible(true)
            self.m_atlasScore:setVisible(false)
            goldImg:setVisible(false)
        else
            text_curScore:setVisible(false)
    ...
    --通用语音按钮
    if not GlobalUserItem.isAntiCheat() and GlobalUserItem.bPrivateRoom then
...
function GameViewLayer:priGameLayerZorder()
    return TAG_ZORDER.PRI_ROOM_INFO
end
--设置名次信息
function GameViewLayer:setWinOrder(logicId, winOrder)
    if logicId <0 or logicId >3 or winOrder == nil or winOrder <= 0 then
        return
    end

    local viewId = self._scene:SwitchViewChairID(logicId)
    self.m_tabNodeCards[viewId]:setWinOrderInfo(winOrder)
end
...
function GameViewLayer:reSetGame()
    ...
    for i = 1, cmd.PLAYER_COUNT do
        ...
        local caiShuScore = self._scene:getCaiShuScore(i)
        local text_curScore = head_bg:getChildByName("text_curScore")
        text_curScore:setString("当前得分:"..self.m_historyScore[i].lCollectScore)
...
function GameViewLayer:reSetForNewGame()
    ...
    for k,v in pairs(self.m_tabNodeCards) do
        ...
        self.m_tabSpAlarm[k]:setSpriteFrame(yl.GetPublicBlankFrame())
    end
...
function GameViewLayer:reSetUserState()
    ...
    for k,v in pairs(self.m_tabStateSp) do
        v:setSpriteFrame(yl.GetPublicBlankFrame())
    end
...
function GameViewLayer:onButtonClickedEvent(tag, ref)
    local function addBG()
        ...
        self:addChild(bg, TAG_ZORDER.SET_ZORDER)
...
--        end
        ...
        self:getParentNode():getParentNode():popHelpLayer2(cmd.KIND_ID,nType,10)
    ...
    elseif TAG_ENUM.BT_CHAT == tag then             --聊天        
        if nil == self.m_chatLayer then
            ...
            self:addChild(self.m_chatLayer, TAG_ZORDER.CHAT_ZORDER)
        end
...
--静音按钮
        ...
        if (self._tag == 0) and not ((GlobalUserItem.nMusic == 100) and (GlobalUserItem.nSound == 100)) then
            self._tag = 1
        end
    ...
    elseif TAG_ENUM.BT_INVITE == tag then       -- 邀请
        ...
        local clicktype = 1
        self:getParentNode():getParentNode():popTargetShare(clicktype, function(target, bMyFriend)
    ...
    elseif TAG_ENUM.BT_OUTCARD == tag then      --出牌
        ...
        self:getParentNode():compareWithLastCards(sel, cmd.MY_VIEWID)               -- 扑克对比
        self:hideOutCardsBtns()
...
--设置托管按钮可用
function GameViewLayer:setTuoGuanBtn(enable)
   self:changeBtnState(self.m_bt_trusteeship,enable)
end
...
function GameViewLayer:outCardEffect(outViewId, outCards, vecCards)
    ...
    local effectPosX = 0 + 667
    local effectPosY = - yl.HEIGHT * 0.1 + 375
    ...
    elseif cmd.LEFT_VIEWID == outViewId then
        ...
        effectPosX = -yl.WIDTH * 0.32 + 667
        effectPosY = yl.HEIGHT * 0.1 + 375
    ...
    elseif cmd.TOP_VIEWID == outViewId then
        ...
        effectPosX = 0 + 667
        effectPosY = yl.HEIGHT * 0.3 + 375
    end
    effectPosX = 0 + 667
    effectPosY = yl.HEIGHT * 0.05 + 375
...
--按照key值给出牌的数组排序
function GameViewLayer:sortCardByKey(vecCard)
end

--改变出牌按钮状态
function GameViewLayer:onChangeOutCardsBtnState( bEnable )
    self.m_bt_outCard:setEnabled(bEnable)
    if bEnable then
...
function GameViewLayer:onPassOutCard()
    ...
    -- 提示
   -- self.m_spInfoTip:setSpriteFrame(yl.GetPublicBlankFrame())
    -- 显示不出
    --[[
...
function GameViewLayer:onSelectedCards( selectCards, cardsNode )
    ...
    local outCards = self:getParentNode().m_tabCurrentCards --上家出的牌
    local outCount = #outCards
    ...
    self:onChangeOutCardsBtnState(enable)
end

-- 牌数变动
-- @param[outCards]         出牌数据
-- @param[cardsNode]        扑克节点,手牌
...
function GameViewLayer:onPromptOut( bOutCard )
    ...
    if bOutCard then
        ...
        if promptCount > 0 then
            ...
            local outCards = self:getParentNode().m_tabCurrentCards --上家出的牌
            local outCount = #outCards
            local promptType = GameLogic:GetCardType(promptCard, #promptCard)

            print("===================托管出牌cardsCount ============================", outCount)
            local enable = false
            if 0 == outCount then
                if  GameLogic.CT_ERROR ~= promptType then
                    enable = true
                end        
            elseif GameLogic:CompareCard(outCards, outCount, promptCard, #promptCard) then
                enable = true
            end
            ...
            if  enable == true then
                ...
                local count = #self.m_tabNodeCards[cmd.MY_VIEWID].m_vecCard
                local handCount = #self.m_tabNodeCards[cmd.MY_VIEWID]:getHandCards()

                self:outCardEffect(cmd.MY_VIEWID, promptCard, vec)
                count = #self.m_tabNodeCards[cmd.MY_VIEWID].m_vecCard
                handCount = #self.m_tabNodeCards[cmd.MY_VIEWID]:getHandCards()
        ...
        if 0 ~= self.m_promptIdx then
            -- 提示回位
            local sel = self.m_tabNodeCards[cmd.MY_VIEWID]:getSelectCards()
            if #sel > 0 and self.m_tabNodeCards[cmd.MY_VIEWID].m_bSuggested and #self:getParentNode().m_tabPromptList > 1 then
...
--设置托管层可见
function GameViewLayer:setGameTrusteeship(bTrustee)
   if self.m_trusteeshipBg then
       self.m_trusteeshipBg:setVisible(bTrustee)
   end
   if self.m_trusteeshipControl then
      self.m_trusteeshipControl:setVisible(bTrustee)
   end
  
   self.m_bTrustee = bTrustee
end

--设置是否托管
function GameViewLayer:onGameTrusteeship( bTrusteeship )
    self:setGameTrusteeship(bTrusteeship)

    if bTrusteeship then
        self:hideOutCardsBtns()
        if self.m_bMyOutCards then
            self.m_bMyOutCards = false
            self:onPromptOut(true) --客户端托管自动出牌
        end
    end
    local cbTrustee = 1
...
function GameViewLayer:updateClock( clockId, cbTime)
    ...
    if cbTime <= 0 then
        ...
        elseif cmd.TAG_COUNTDOWN_OUTCARD == clockId then
            -- 私人房无自动托管
            if self.m_nCountDownView == cmd.MY_VIEWID then  --not GlobalUserItem.bPrivateRoom  and 
...
function GameViewLayer:OnUpdateUser(viewId, userItem, bLeave)
    ...
    if nil == self.m_tabUserItem[viewId] then
        ...
        --玩家当前游戏得分
        local caiShuScore = self._scene:getCaiShuScore(i)
        print("1111111111111111111111111111111111111",caiShuScore)
        local text_curScore = head_bg:getChildByName("text_curScore")
        text_curScore:setString("当前得分:"..self.m_historyScore[viewId].lCollectScore)
        if self._scene.m_bIsYueZhan == true then
            text_curScore:setVisible(true)
...
function GameViewLayer:onGameStart()
    ...
    --庄家,显示心，就是庄家
    local bankView = self._scene:SwitchViewChairID(self._scene.m_wBankerUser)               ---逻辑坐标转成视图坐标 
...
--显示伙伴标记
function GameViewLayer:showPartner()
    if self._scene.m_wBankerUser ~= self._scene.m_wPartner then
        local partnerView = self._scene:SwitchViewChairID(self._scene.m_wPartner)      
        local head_bg = self.m_csbNode:getChildByName(string.format("head_bg_%d", partnerView))
        local flag = head_bg:getChildByName("imgFlag")
        flag:setVisible(true)
        flag:loadTexture("game_res/play_huoban.png")
    end
end
--显示庄家
function GameViewLayer:showBankUser()
    if self._scene.m_wBankerUser ~= self._scene.m_wPartner then
        local partnerView = self._scene:SwitchViewChairID(self._scene.m_wBankerUser)      
        local head_bg = self.m_csbNode:getChildByName(string.format("head_bg_%d", partnerView))
        local flag = head_bg:getChildByName("imgFlag")
        flag:setVisible(true)
        flag:loadTexture("game_res/play_zhuang.png")
    end
end
--首出牌的提示情况
function GameViewLayer:onGameFirstOutCards()
    if self._scene.m_wCurrentUser == self.m_wChairId then
        -- 不出按钮
        self:onChangePassBtnState(false)
        local handCards = self.m_tabNodeCards[cmd.MY_VIEWID]:getHandCards()
        local outCard = handCards[1]
        self._scene:updatePromptList({}, handCards, cmd.MY_VIEWID, cmd.MY_VIEWID)
        self:presentOutCardsBtns()
        self.m_bMyOutCards = true
    end
end
...
function GameViewLayer:onGetGameCard(viewId, cards, bReEnter, pCallBack)
    ...
    if bReEnter then    
        ...
        local call = cc.CallFunc:create(function()
            ...
            self.m_tabNodeCards[cmd.LEFT_VIEWID]:updateCardsNode(empTyCard, true, true)
            self.m_tabNodeCards[cmd.RIGHT_VIEWID]:updateCardsNode(empTyCard, true, true)
            self.m_tabNodeCards[cmd.TOP_VIEWID]:updateCardsNode(empTyCard, true, true)
        ...
        local seq = cc.Sequence:create(call2, call)   --cc.DelayTime:create(0.3),
        self:stopAllActions()
        self:runAction(seq)
    end
end

--托管情况下的自动出牌(是显示服务端传过来的数据)
function GameViewLayer:outCardInTrustee(curViewId, lastViewId, outCards)
...
function GameViewLayer:onGetOutCard(curViewId, lastViewId, lastOutCards, bReEnter)
    ...
    if curViewId ~= cmd.INVALID_CHAIRID then
        ...
        if nil ~= self.m_tabStateSp[curViewId] then
            self.m_tabStateSp[curViewId]:setSpriteFrame(yl.GetPublicBlankFrame())
        end
        --]]
        ...
        if curViewId == cmd.MY_VIEWID then
            self:presentOutCardsBtns()
            self:onChangeOutCardsBtnState(false)
            
            if self.m_bTrustee == true then                         --自动提示出牌
                ...
                --self.m_bCanOutCard = (#promptList > 0)  --这是提示列表里的判断
                ...
                if not self.m_bCanOutCard then
                    ...
                    self:onChangeOutCardsBtnState(true)
                    local sel = self.m_tabNodeCards[cmd.MY_VIEWID]:getSelectCards()
                    local selCount = #sel
                    if selCount > 0 then
...
function GameViewLayer:onGetPassCard( dataBuffer )
    ...
    if 1 == dataBuffer.cbTurnOver then
        ...
        --local text_curScore = head_bg:getChildByName("text_curScore")
        --text_curScore:setString(string.format("当前得分:%d", dataBuffer.TurnScore))

        --算总分
        self.m_nCurTotalScore = self.m_nCurTotalScore + dataBuffer.TurnScore
        --self.m_nCurTotalScore = 0
        self.m_round_outTotalScore:setString(string.format("%d", self.m_nCurTotalScore))
        for i = 1, cmd.PLAYER_COUNT do
            self.m_outCardsControl:removeChildByTag(i)
...
function GameViewLayer:onGetGameConclude( rs )
    ...
    -----------------------------历史记录-------------------
    if self._scene.m_bIsYueZhan == false then
        self.m_lGameScore = rs.lGameScore[1]
        self.m_cbScore = rs.PlayerScore[1]
        for i = 1 ,cmd.PLAYER_COUNT do
            local viewId = self._scene:SwitchViewChairID(i - 1)
         ...
         for i = 1 ,cmd.PLAYER_COUNT do
             local viewId = self._scene:SwitchViewChairID(i - 1)
             self.m_lCollectScore[i]  =  self.m_historyScore[viewId].lCollectScore + rs.lRoundScore[1][i]
             local userItem = self.m_historyUseItem[viewId]
             if nil == userItem then
                 userItem = self.m_tabUserItemCopy[viewId]
             end
             --历史成绩
            self.m_historyScore[viewId].lTurnScore = rs.lRoundScore[1][i]
            self.m_historyScore[viewId].lCollectScore = self.m_historyScore[viewId].lCollectScore + rs.lRoundScore[1][i]
            self.m_outCardsControl:removeChildByTag(viewId)
         end
    end
    ...
    if nil == self.m_resultLayer then
        ...
        self.m_resultLayer:setAnchorPoint(cc.p(0.5,0.5))
        self.m_resultLayer:setPosition(cc.p(0,0))                   --680,410
    ...
    if self._scene.m_bIsYueZhan == false then
       for i = 1, cmd.PLAYER_COUNT do
            ...
            if nil ~= self.m_tabUserItem[viewId] then
                ...
                local player_score = head_bg:getChildByName("player_score")                     --玩家游戏得分
                local goldImg = head_bg:getChildByName("gold_img")
                player_score:setString(string.format("%d",userItem.lScore))

                local caiShuScore = self._scene:getCaiShuScore(i)
                local text_curScore = head_bg:getChildByName("text_curScore")
                text_curScore:setString("当前得分:"..self.m_historyScore[i].lCollectScore)               --当前游戏得分 self.m_historyScore[viewId].lCollectScore
            ...
            self.m_cbScore[i] = 0
            self.m_tabNodeCards[viewId]:emptyWinOrderInfo()
        end
    else
        for i = 1, cmd.PLAYER_COUNT do
            local viewId = self._scene:SwitchViewChairID(i - 1)
            ...
            local caiShuScore = self._scene:getCaiShuScore(i)
            local text_curScore = head_bg:getChildByName("text_curScore")                       --当前游戏得分
            text_curScore:setString("当前得分:"..self.m_historyScore[i].lCollectScore)                       --self.m_historyScore[viewId].lCollectScore
            local goldImg = head_bg:getChildByName("gold_img")
            local player_score = head_bg:getChildByName("player_score") 
            text_curScore:setVisible(true)
            player_score:setVisible(false)
...
function GameViewLayer:updateRule()
    --4,6王
    local text_rule = self.left_bg_normal:getChildByName("text_rule")
    if self._scene.m_ModelWang == 1 then
        text_rule:setString("4王")
    else
        text_rule:setString("6王")
    end
...
--不出提示
function GameViewLayer:setOperateTipVisible(viewId,bIsVisible)
...
function GameViewLayer:initCardsNodeLayout()
    for index = 2, cmd.PLAYER_COUNT do 
        ...
        for i = 1, cmd.NORMAL_COUNT do
            ...
            --card:setTextureRect(cc.rect(0 * CARD_WIDTH_SMALL, 4 * CARD_HEIGHT_SMALL, CARD_WIDTH_SMALL, CARD_HEIGHT_SMALL))
            card:setTextureRect(cc.rect(2 * CARD_WIDTH_SMALL, 4 * CARD_HEIGHT_SMALL, CARD_WIDTH_SMALL, CARD_HEIGHT_SMALL))
