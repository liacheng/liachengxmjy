...
function ApplyListLayer:ctor(viewParent)
    ...
    self.m_pTextName:setFontName("fonts/round_body.ttf")
    ...
    self.m_pIconArrow = csbNode:getChildByName("Image_1")
    ...
    for i = 1, #self.m_userlist do
        ...
        if nil ~= useritem then
            if i==1 then
                ...
                self.m_pTextCoin:setString(ExternalFun.formatScoreText(useritem.lScore))
            end
...
function ApplyListLayer:onButtonClickedEvent( tag, sender )
	...
	if ApplyListLayer.BT_SHOW == tag then
        ...
        self.m_pIconArrow:setVisible(false)
        ExternalFun.showLayer(self, self.m_pIconBg)
        self.m_pScrollView:removeAllChildren()
        self:reload()
    elseif ApplyListLayer.BT_HIDE == tag then
        self.m_pBtnShow:setVisible(true)
        self.m_pIconArrow:setVisible(true)
        ExternalFun.hideLayer(self, self.m_pIconBg, false)
        self.m_pScrollView:removeAllChildren()
        self:reload()
	end
end
