...
function GameCardLayer:dispatchUpdate( )
	if 0 ~= #self.m_vecDispatchCards then
		...
		--will--if nil ~= self.m_parent then
...
function GameCardLayer:calResult( )
	...
	--点数记录
	self.m_parent:getDataMgr().m_tabGameResult.m_cbPlayerPoint = idlePoint
	self.m_parent:getDataMgr().m_tabGameResult.m_cbBankerPoint = masterPoint
