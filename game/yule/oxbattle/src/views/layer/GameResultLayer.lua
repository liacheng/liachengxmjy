...
--zy	local pname = temp:getChildByName("txt_banker")
...
--	self.m_winBankerName = clipText
	...
	--玩家名称
--zy	pname = temp:getChildByName("txt_self")
...
--	self.m_winmaohao1 = temp:getChildByName("txt_maohao_0")
    self.m_winImBack = csbNode:getChildByName("im_light")
end
...
--	self.m_failedSelfName = clipText
    ...
    self.m_faileImBack = csbNode:getChildByName("im_light")
end
...
function GameResultLayer:showGameWin()
    ...
    self.m_winImBack:runAction(cc.RepeatForever:create(cc.RotateBy:create(4, 360)))
...
--	self.m_winBankerName:setString(self.m_bankername)
    ...
    elseif self.m_bankerscore < 0 then
        bankerstr = ExternalFun.numberThousands(math.abs(self.m_bankerscore))
        bankerstr = "/"..bankerstr
	end
...
--	self.m_winSelfName:setString(self.m_selfname)
    ...
    elseif self.m_selfscore < 0 then
        scorestr = ExternalFun.numberThousands(math.abs(self.m_selfscore))
        scorestr = "/"..scorestr
	end
...
--	end
	...
	ExternalFun.playSoundEffect("oxbattle_game_win.mp3")
end
...
function GameResultLayer:showGameFailed()
    ...
    self.m_faileImBack:runAction(cc.RepeatForever:create(cc.RotateBy:create(4, 360)))
...
--	end
	...
	ExternalFun.playSoundEffect("oxbattle_game_lose.mp3")
end
