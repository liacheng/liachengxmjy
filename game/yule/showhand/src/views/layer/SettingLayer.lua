...
SettingLayer.RES_PATH 				= "game/yule/showhand/res/"
...
function SettingLayer:preloadUI()
    cc.Director:getInstance():getTextureCache():addImageAsync("set/showhand_setlayer.png", function (args)
...
function SettingLayer:ctor(version)
	...
	--区域外取消显示
	local onExitTouch = function(eventType, x, y)
		if not self._canTouchOutside or self._isDiss then
			return true
		end

		if eventType == "began" then
			local rect = self:getChildByTag(SettingLayer.TAG_BACKGROUND):getBoundingBox()
	...
	self:registerScriptTouchHandler(onExitTouch)
    ...
    -- 背景
    rootLayer, self._setLayer = ExternalFun.loadRootCSB(SettingLayer.RES_PATH.."set/set.csb", self)

    self.setBg = self._setLayer:getChildByName("bg")
    self.setBg:setLocalZOrder(5)
    self.setBg:setTag(SettingLayer.TAG_BACKGROUND)

    self.setTitle = self._setLayer:getChildByName("icon_help")
    ...
    -- 音乐按钮-关
    -- 音乐文字   
	ccui.Button:create("showhand_set_icon_open.png", "showhand_set_icon_open.png","showhand_set_icon_open.png",UI_TEX_TYPE_PLIST)
        :move(580, 360)
        :setTag(SettingLayer.TAG_MUSIC_OPEN)
        :addTo(self)
        :addTouchEventListener(onBtnCallBack)

	ccui.Button:create("showhand_set_icon_close.png", "showhand_set_icon_close.png","showhand_set_icon_close.png",UI_TEX_TYPE_PLIST)
        :move(580, 360)
        :setTag(SettingLayer.TAG_MUSIC_CLOSE)
        :addTo(self)
        :addTouchEventListener(onBtnCallBack)        

    display.newSprite("#showhand_set_icon_music.png")
    ...
    display.newSprite("#showhand_set_icon_sound.png")
		:setTag(SettingLayer.TAG_BACKGROUND)
		:move(740,360)
		:addTo(self)

    -- 关闭按钮
    ccui.Button:create("showhand_set_btn_close.png","showhand_set_btn_close.png","showhand_set_icon_close.png",UI_TEX_TYPE_PLIST)
        :move(910, 510)
...
function SettingLayer:onClose()
    ExternalFun.hideLayer(self, self)
	self._isDiss = true
	--self:stopAllActions()
	...
	cc.Director:getInstance():getTextureCache():removeTextureForKey("set/showhand_setlayer.png")
end
