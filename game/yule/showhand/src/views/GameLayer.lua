...
local cmd = appdf.req(appdf.GAME_SRC.."yule.showhand.src.models.CMD_Game")
local GameLogic = appdf.req(appdf.GAME_SRC.."yule.showhand.src.models.GameLogic")
local GameViewLayer = appdf.req(appdf.GAME_SRC.."yule.showhand.src.views.layer.GameViewLayer")
...
function GameLayer:onEventGameScene(cbGameStatus,dataBuffer)
	...
	if cbGameStatus == cmd.GAME_SCENE_FREE	then			    --空闲状态
       ...
       local cmd_table = ExternalFun.read_netdata(cmd.CMD_S_StatusFree, dataBuffer)
        ...
        self._gameView.m_EveryBottomScore:setString("")
--        self._gameView:clearCardBox()
	...
	elseif cbGameStatus == cmd.GAME_SCENE_PLAY	then			--游戏状态
        ...
        self.m_lUserScore = {0,0}
        self.m_lTableScore = {0,0,0,0,0}
        self.m_giveUpPlayer = {0,0,0,0,0}
        --self._gameView.m_EveryBottomScore:setString(self.m_lCellScore)
        local str = string.format("底注:%d", self.m_lCellScore)
        self._gameView.m_EveryBottomScore:setString(str)
...
--        self._gameView:clearCardBox()
--        self._gameView:UpdataCardBox(self._gameView.m_spriteCard)
        ...
        if self.m_lTotalScore[self.m_wAddUser+1]  >= self.m_lDrawMaxScore then --梭哈           
            local viewid = self:SwitchViewChairID(self.m_wAddUser)
...
--    self._gameView:UpdataCardBox(self._gameView.m_spriteCard)
    ...
    --初始化已有玩家数据
    for i = 1, cmd.GAME_PLAYER do
        local userItem = self._gameFrame:getTableUserItem(self._gameFrame:GetTableID(), tonumber(i-1))
        if nil ~= userItem then
    ...
    --音效
    ExternalFun.playSoundEffect("showhand_game_start.wav")
end
...
function GameLayer:onSubAddScore(dataBuffer)
    ...
    if cmd_table.lUserScoreCount ~= 0 then 
        ...
        elseif cmd_table.lUserScoreCount == self.m_llStartScore then --跟注
            ...
            ExternalFun.playSoundEffect("showhand_follow.wav")
        else --加注
            self._gameView:showTip(viewid,3)
            ExternalFun.playSoundEffect("showhand_add_score.wav")
        end
    else --不加
        self._gameView:showTip(viewid,1)
        ExternalFun.playSoundEffect("showhand_no_add.wav")
    end
    --底部显示
    self:updateOperatorText(self.m_wCurrentUser)
...
function GameLayer:onGameSceneGiveUp(dataBuffer)
    ...
    ExternalFun.playSoundEffect("showhand_give_up.wav")
end
...
function GameLayer:onSubGameEnd(dataBuffer)
    ...
    self._gameView:showAddSlider(false)
...
function GameLayer:ShowGameEnd()
    ...
    self._gameView:removeShowHandRun()
    --音效
    if self.m_winnerChairId == (MyChair+1) then
        ExternalFun.playSoundEffect("showhand_game_win.wav")
    else
        ExternalFun.playSoundEffect("showhand_game_lose.wav")
    end
    ...
    self:SetGameClock(self:GetMeChairID(),cmd.IDI_START_GAME,cmd.TIME_START_GAME)
    self.m_tEndcmdTable = nil    
    ExternalFun.playSoundEffect("showhand_game_end.wav")
end
...
function GameLayer:onGiveUp()
    ...
    self._gameView:showOperateButton(false)    
    self._gameView:setTouchEnabled(false)
    self._gameView:showAddSlider(false)
    --发送数据
...
function GameLayer:onSlideAddScore()
    ...
    if self._gameView.addChipChatScore:getString() ~= "" then
        addScore = tonumber(self._gameView.addChipChatScore:getString())+self.m_llLastScore
    end
...
function GameLayer:onShowHand()
    ...
    self:KillGameClock()
    self._gameView:showAddSlider(false)
    self._gameView.btnShowHand:setVisible(false)
    --下注金额
...
function GameLayer:UpdataControl()
    ...
    self._gameView:setBtnEnabled(self._gameView.btnAdd,true)
    self._gameView.btnAddImg:loadTexture("showhand_btntab_addChip.png",1)
    ...
    for i=1,4 do
        if maxScore  < self.m_lCellScore*i then
            ...
            self._gameView:setBtnEnabled(self._gameView.btChip[i],false)
            ...
            self._gameView:setBtnEnabled(self._gameView.btChip[i],true)
    ...
    if maxScore < self.m_lCellScore and self.m_llLastScore > 0 then     
        self._gameView:setBtnEnabled(self._gameView.btnAdd,false)
        self._gameView.btnAddImg:loadTexture("showhand_btntab_addChip1.png",1)
        if self.m_sendCardCount >3 then
           --self._gameView.btnFollow:setVisible(false)
           self._gameView.btnAdd:setVisible(false)
           self._gameView.btnShowHand:setVisible(true)   
        end       
    end
end
...
function GameLayer:onAddScoreButton()
   ...
   -- self._gameView.m_slider:setVisible(true)
    ...
    self._gameView:showAddSlider(true)
...
function GameLayer:SetGameClock(chair,id,time)
    ...
    if viewid and viewid ~= yl.INVALID_CHAIR then               
        --self._gameView.m_clock[viewid]:setVisible(true) 
        --local str = string.format("%d",time)
        --self._gameView.m_clock[viewid]:getChildByName("time"):setString(str)
--        if viewid == 2 then
--            if cmd.IDI_START_GAME == id then 
--                self._gameView.m_clock[viewid]:setPosition(cc.p(yl.DESIGN_WIDTH/2,380))
--            elseif cmd.IDI_USER_ADD_SCORE == id then
--                self._gameView.m_clock[viewid]:setPosition(cc.p(274,160))
--            end
--        end 
...
function GameLayer:OnEventGameClockInfo(chair,time,clockid)
    if time < 5 then
        ExternalFun.playSoundEffect("showhand_game_warn.wav")
    end
    ...
    if MeItem.lScore == 0 then
        ...
        local seq = cc.Sequence:create(
            cc.CallFunc:create(function (  )
                showToast(cc.Director:getInstance():getRunningScene(), "金币不足，无法继续游戏",1.5)
            end),
