...
function Bullet:ctor(angle,chairId,mutiple,CannonType,cannon,android_chairId,bulletSpeedIndex,isSelf)
	...
	self.m_updateTime = self._dataModule.bullet_update_[bulletSpeedIndex]
	if isSelf then
     ...
     --/ self._dataModule.bullet_speed_Mutiple  -- 子弹刷新时间
...
function Bullet:setType(wChairID, type)
    ...
    local sBulletName = string.format("#zyqs_bullet%d_norm%d.png", nBulletNum, wChairID + 1)
    if self.m_Type == g_var(cmd).CannonType.Bullet_Special_Cannon then
        if wChairID + 1 <=2 then
            sBulletName = string.format("#zyqs_bullet%d_ion%d.png", nBulletNum, 1)
        else
            sBulletName = string.format("#zyqs_bullet%d_ion%d.png", nBulletNum, 2)
        end
    end
...
function Bullet:changeDisplayFrame( chairId , score)
	local nBulletNum = self:getBulletNum()
	local frame = string.format("zyqs_bullet%d_norm%d.png", nBulletNum,chairId + 1)
