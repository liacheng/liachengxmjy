...
function GameLayer:onEventUserScore(item)   -- 用户分数
    print("fishzyqs onEventUserScore...")
end
        ...
        for i=1,fishNum do
            ...
            if FishCreate.fish_kind >= g_var(cmd).FishKind.FISH_KIND_18 and FishCreate.fish_kind <= g_var(cmd).FishKind.FISH_KIND_21 then
                ...
                elseif FishCreate.fish_kind == g_var(cmd).FishKind.FISH_KIND_19 then
                    groupTipsName = "game_res/zyqs_bg_dragon.png"
                ...
                elseif FishCreate.fish_kind == g_var(cmd).FishKind.FISH_KIND_20 then
                    groupTipsName = "game_res/zyqs_bg_lg.png"
                    fishMutiplesStr = "320"
                    fishMutiplesPos = cc.p(yl.WIDTH/2-380+80,yl.HEIGHT/2-35)

                elseif FishCreate.fish_kind == g_var(cmd).FishKind.FISH_KIND_21 then
                    groupTipsName = "game_res/zyqs_bg_tulongdao.png"
                    fishMutiplesStr = "40/300"
                    fishMutiplesPos = cc.p(yl.WIDTH/2-580+80,yl.HEIGHT/2-35)

                end

                local tipNode = cc.Node:create()
                local groupTips = display.newSprite(groupTipsName)
                local fishmultiples = cc.LabelAtlas:create(fishMutiplesStr,"game_res/zyqs_num_bigfish.png",54,75,string.byte("/"))   -- 鱼的倍数
                local multiple = display.newSprite("game_res/zyqs_icon_multiple.png")
                local horizon = display.newSprite("game_res/zyqs_bg_horizon.png")
                ...
                if FishCreate.fish_kind == g_var(cmd).FishKind.FISH_KIND_21  then
                    multiple:setPosition(cc.p(yl.WIDTH/2-180+80,yl.HEIGHT/2))
                elseif FishCreate.fish_kind == g_var(cmd).FishKind.FISH_KIND_19 then
                ...
                elseif FishCreate.fish_kind == g_var(cmd).FishKind.FISH_KIND_21 then
                    horizon:setPosition(cc.p(yl.WIDTH/2+253+80,yl.HEIGHT/2))
                else
                    horizon:setPosition(cc.p(yl.WIDTH/2+220+80,yl.HEIGHT/2))
                end
                ...
                if layer == nil then
                    ExternalFun.playSoundEffect(g_var(cmd).BigFishComing)
                end
...
--            end
            ...
            if nil ~= fish then
                ...
                local CatchFishSound = string.format("fishzyqs4_catch_fish_%d.mp3", random)
