...
GameViewLayer.RES_PATH = device.writablePath.. "game/yule/fishdntg/res/"
...
--底部框隐藏
--    local bgScore = display.newSprite("#dntg_bg_lowarticle.png")
...
--    self.btnMinus:setPosition(yl.WIDTH - 64 - xoffset, 200)
    ...
    --[[if self._scene.m_nChairID == 0 then
        self.btnAdd:setPosition(985+xexcursion, 27)
        self.btnMinus:setPosition(985-182+xexcursion, 27)
    elseif self._scene.m_nChairID == 1 then
        self.btnAdd:setPosition(532+xexcursion, 27)
        self.btnMinus:setPosition(532-182+xexcursion, 27)
...
--        btnAdd:setPosition(722, 21)
--        btnMinus:setPosition(715-112, 21)
--    elseif self._scene.m_nChairID == 5 then
--        btnAdd:setPosition(1132, 21)
--        btnMinus:setPosition(1130-112, 21)
...
--    self:addChild(autoLockBG,20)
    ...
    self.autoShootBtn = ccui.Button:create("dntg_btn_auto.png","dntg_btn_auto.png","dntg_btn_auto.png",UI_TEX_TYPE_PLIST)      -- 自动射击
...
--    self.autoShootBtn:setScaleY(0.9)
    ...
    self.autoLockBtn = ccui.Button:create("dntg_btn_lock.png","dntg_btn_lock.png","dntg_btn_lock.png",UI_TEX_TYPE_PLIST)       -- 自动锁定
...
--    autoLockBtn:setScaleY(0.9)
    ...
    self.speedUpBtn = ccui.Button:create("dntg_btn_speed1.png","dntg_btn_speed1.png","dntg_btn_speed1.png",UI_TEX_TYPE_PLIST)       -- 加速
...
--    self.speedUpBtn:addChild(label3,3)
    ...
    --添加按钮背景
--    self.menuSetBtnBg = display.newSprite("#dntg_btntable_down.png")
--    self.menuSetBtnBg:setAnchorPoint(0.5,1)
--    self.menuSetBtnBg:setPosition(58, yl.HEIGHT -11)
--    self:addChild(self.menuSetBtnBg,21)

--    self.menuSetBtnBg2 = display.newSprite("#dntg_btntable_down.png")
...
--    self.menuSetBg = ccui.ImageView:create("dntg_bg_down.png",UI_TEX_TYPE_PLIST)      -- 添加菜单背景
...
--    self.menuSetBg:addChild(bank)
    ...
    --按钮收缩菜单按钮
--    self.shrinkmenu = ccui.Button:create("dntg_btn_down.png","dntg_btn_down.png","dntg_btn_down.png",UI_TEX_TYPE_PLIST)
...
--    self:addChild(self.shrinkmenu,23)
    ...
    --按钮收缩菜单箭头
--    self.shrinkmenuarrows = display.newSprite("#dntg_btn_downarrow.png")
...
--    self.shrinkmenu:addChild(self.shrinkmenuarrows)
    ...
    self.menu = ccui.Button:create("dntg_btn_down.png","dntg_btn_down.png","dntg_btn_down.png",UI_TEX_TYPE_PLIST)               -- 菜单
    ...
    --菜单箭头
    self.menuarrows = display.newSprite("#dntg_btn_downarrow.png")
    self.menuarrows:setAnchorPoint(0.5,0.5)
    self.menuarrows:setPosition(50,48)
    self.menu:addChild(self.menuarrows)

    self.help = ccui.Button:create("dntg_btn_help.png","dntg_btn_help.png","dntg_btn_help.png",UI_TEX_TYPE_PLIST)
    self.help:setTag(2)
    ...
    self.set = ccui.Button:create("dntg_btn_set.png","dntg_btn_set.png","dntg_btn_set.png",UI_TEX_TYPE_PLIST)
    self.set:setTag(3)
    ...
    self.clear = ccui.Button:create("dntg_btn_turnback.png","dntg_btn_turnback.png","dntg_btn_turnback.png",UI_TEX_TYPE_PLIST)
    self.clear:setTag(4)
...
--    self:addChild(nick,22)
    ...
    --用户分数 
--    local score = cc.Label:createWithCharMap("game_res/dntg_num_small.png",18,25,string.byte("%"))
--用户的分数隐藏
...
--    local score = cc.LabelAtlas:create(num,"game_res/dntg_num_small.png",18,25, string.byte("*"))
...
function GameViewLayer:setAutoShoot(b,target)
    if b then
        ...
        target:loadTextureNormal("dntg_btn_autoing.png",UI_TEX_TYPE_PLIST);
        target:loadTexturePressed("dntg_btn_autoing.png",UI_TEX_TYPE_PLIST);
    else
        self.tempNode:setVisible(false)
        target:loadTextureNormal("dntg_btn_auto.png",UI_TEX_TYPE_PLIST);
        target:loadTexturePressed("dntg_btn_auto.png",UI_TEX_TYPE_PLIST);
    end
end
...
function GameViewLayer:setAutoLock(b,target)
    if b then
        ...
        target:loadTextureNormal("dntg_btn_locking.png",UI_TEX_TYPE_PLIST);
        target:loadTexturePressed("dntg_btn_locking.png",UI_TEX_TYPE_PLIST);
    else
        self.tempLockNode:setVisible(false)
        target:loadTextureNormal("dntg_btn_lock.png",UI_TEX_TYPE_PLIST);
        target:loadTexturePressed("dntg_btn_lock.png",UI_TEX_TYPE_PLIST);
        --取消自动射击
...
function GameViewLayer:setSpeedUP(mutiple,target)
    ...
    local TextureName = string.format("dntg_btn_speed%d.png",mutiple)
...
--                self.btncombination:setSelected(self.m_control3)
    ...
    elseif 3 == tag then --设置
        ...
        if GlobalUserItem.nMusic == 100 then
            muteBtnVoice:loadTextureNormal("dntg_btn_setopen.png",UI_TEX_TYPE_PLIST)
        end

        if GlobalUserItem.nSound == 100 then
            muteBtn:loadTextureNormal("dntg_btn_setopen.png",UI_TEX_TYPE_PLIST)   
        end

        if GlobalUserItem.bShake == 1 then
            btnShake:loadTextureNormal("dntg_btn_setopen.png", UI_TEX_TYPE_PLIST) 
        end
        ...
        muteBtn:addTouchEventListener(function( sender,eventType ) -- 音效按钮
            if eventType == ccui.TouchEventType.ended then
                ...
                if GlobalUserItem.nSound == 100 then
                    ...
                    muteBtn:loadTextureNormal("dntg_btn_setclose.png",UI_TEX_TYPE_PLIST)
                end
            end
        end)
        ...
        muteBtnVoice:addTouchEventListener(function( sender,eventType ) -- 音乐按钮
            if eventType == ccui.TouchEventType.ended then
                ...
                if GlobalUserItem.nMusic == 100 then
                    ...
                    muteBtnVoice:loadTextureNormal("dntg_btn_setclose.png",UI_TEX_TYPE_PLIST)
                end
            end
        end)
        ...
        btnShake:addTouchEventListener(function(sender,eventType)       -- 震屏按钮
            if eventType == ccui.TouchEventType.ended then
                ...
                if GlobalUserItem.bShake == 1 then
                    ...
                    btnShake:loadTextureNormal("dntg_btn_setclose.png",UI_TEX_TYPE_PLIST)
                end
            end
        end)
...
function GameViewLayer:ShowParticle(fishtype, pos, tag)
    if fishtype >= 15 and fishtype <= 23 then
        -- 播放金币
        local particle = cc.ParticleSystemQuad:create("particle/bigwin_blowout_1.plist")
        particle:setPosition(cc.p(pos.x, pos.y))
        self._scene.m_cannonLayer:addChild(particle)
        local call = cc.CallFunc:create(function()
            particle:removeFromParent()
        end)
        self._scene.m_cannonLayer:runAction(cc.Sequence:create(cc.DelayTime:create(2),call))
    elseif fishtype >= 24 and fishtype <= 32 then
        if fishtype == 26 then
            -- 佛手
            local particle = cc.ParticleSystemQuad:create("particle/dntg_particle_foshou1.plist")
            particle:setPosition(cc.p(pos.x, pos.y))
            self._scene.m_cannonLayer:addChild(particle)

            local particle1 = cc.ParticleSystemQuad:create("particle/dntg_particle_foshou2.plist")
            particle1:setPosition(cc.p(pos.x, pos.y))
            self._scene.m_cannonLayer:addChild(particle1)

            local call = cc.CallFunc:create(function()
                particle:removeFromParent()
                particle1:removeFromParent()
            end)
            self._scene.m_cannonLayer:runAction(cc.Sequence:create(cc.DelayTime:create(2),call))
        elseif fishtype == 27 then
            -- 金箍棒
            -- 乾坤圈
            -- 定屏
            if tag == 2 then
                local particle = cc.ParticleSystemQuad:create("particle/dntg_particle_gun1.plist")
                particle:setPosition(cc.p(pos.x, pos.y))
                self._scene.m_cannonLayer:addChild(particle)

                local particle1 = cc.ParticleSystemQuad:create("particle/dntg_particle_gun2.plist")
                particle1:setPosition(cc.p(pos.x, pos.y))
                self._scene.m_cannonLayer:addChild(particle1)
                ...
                self._scene.m_cannonLayer:runAction(cc.Sequence:create(cc.DelayTime:create(2),call))
            elseif tag == 1 then
                local particle = cc.ParticleSystemQuad:create("particle/dntg_particle_quan.plist")
                particle:setPosition(cc.p(pos.x, pos.y))
                self._scene.m_cannonLayer:addChild(particle)

                local particle1 = cc.ParticleSystemQuad:create("particle/dntg_particle_quan2.plist")
                particle1:setPosition(cc.p(pos.x, pos.y))
                self._scene.m_cannonLayer:addChild(particle1)
                ...
                self._scene.m_cannonLayer:runAction(cc.Sequence:create(cc.DelayTime:create(2),call))
            elseif tag == 0 then
                local particle = cc.ParticleSystemQuad:create("particle/dntg_particle_ding.plist")
                particle:setPosition(cc.p(pos.x, pos.y))
                self._scene.m_cannonLayer:addChild(particle)

                local particle1 = cc.ParticleSystemQuad:create("particle/dntg_particle_ding2.plist")
                particle1:setPosition(cc.p(pos.x, pos.y))
                self._scene.m_cannonLayer:addChild(particle1)
            ...
            -- bigfish
            local particle = cc.ParticleSystemQuad:create("particle/dntg_particle_bigfish1.plist")
            particle:setPosition(cc.p(pos.x, pos.y))
            self._scene.m_cannonLayer:addChild(particle)

            local particle1 = cc.ParticleSystemQuad:create("particle/dntg_particle_bigfish2.plist")
            particle1:setPosition(cc.p(pos.x, pos.y))
            self._scene.m_cannonLayer:addChild(particle1)

            local particle2 = cc.ParticleSystemQuad:create("particle/dntg_particle_bigfish3.plist")
            particle2:setPosition(cc.p(pos.x, pos.y))
            self._scene.m_cannonLayer:addChild(particle2)
...
function GameViewLayer:ShowAniChain(wChairID, delayTime)
    ...
    local plight = cc.Sprite:createWithSpriteFrameName("LChainFlag_1.png")
    plight:setPosition(cc.p(cannon:getPositionX(), cannon:getPositionY()))
    plight:setAnchorPoint(cc.p(1.0, 0.5))
    plight:setRotation(rotate)
    self:addChild(plight,6)

    local anim = cc.AnimationCache:getInstance():getAnimation("LChainFlag")
    local panim = cc.RepeatForever:create(cc.Animate:create(anim))
    plight:runAction(panim) 
    plight:runAction(cc.Sequence:create(cc.RotateBy:create(3, 360), cc.DelayTime:create(delayTime-3), cc.RemoveSelf:create())) 
...
function GameViewLayer:ShowCoin2(score, wChairID, fish_pos, fishRotation)
    ...
    local goldNum = cc.LabelAtlas:create(num,"game_res/dntg_num_bigfish.png",54,75,string.byte("/"))
...
function GameViewLayer:RandomCoin(fishPos, coinNum,  cannon, score)
    ...
    for i = 1, coinNum do
        ...
        fishSpr = display.newSprite("#dntg_gold_coin_7.png")
        :addTo(self._scene.m_cannonLayer, 1)
...
function GameViewLayer:showLight3(point1X, point1Y, point2X, point2Y, moveTime)
    ...
    plight1:setPosition(cc.p(point2X, point2Y))
    plight1:setColor(cc.c3b(255, 255, 0))
    plight1:setOpacity(255)
    plight1:setAnchorPoint(cc.p(0.5, 0))
    ...
    local panim = cc.RepeatForever:create(cc.Animate:create(anim))
    plight1:runAction(panim) 
    plight1:runAction(cc.Sequence:create(cc.ScaleTo:create(moveTime, 1, pscaley), cc.RemoveSelf:create())) 
end

function GameViewLayer:showLight1(point1X, point1Y, point2X, point2Y, moveTime)
    local point1 = cc.p(point1X,point1Y)
    local point2 = cc.p(point2X,point2Y)
    self:showBall(point2)
    local distant = cc.pGetDistance(point1, point2)
    ...
    plight2:setPosition(cc.p(point2X, point2Y))
    plight2:setColor(cc.c3b(0, 215, 255))
    plight2:setOpacity(150)
    plight2:setAnchorPoint(cc.p(0.5, 0))
    ...
    local panim = cc.RepeatForever:create(cc.Animate:create(anim))
    plight1:runAction(panim) 
    --plight2:runAction(panim)
    plight1:runAction(cc.Sequence:create(cc.ScaleTo:create(moveTime, 0), cc.RemoveSelf:create())) 
    --plight2:runAction(cc.Sequence:create(cc.ScaleTo:create(moveTime, 0), cc.RemoveSelf:create()))
end
