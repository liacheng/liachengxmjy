...
--    end
    ...
    mutipleNum = 2
    if self.m_nMultiple >= 10000 then
        mutipleNum = 4
    elseif self.m_nMultiple >= 1000 then 
        mutipleNum = 3
    end
...
function Bullet:setType(wChairID, type)
    ...
    local sBulletName = string.format("dntg_bullet%d_norm%d.png", nBulletNum, wChairID + 1)
    if self.m_Type == g_var(cmd).CannonType.Bullet_Special_Cannon then
        if wChairID + 1 <=2 then
            sBulletName = string.format("dntg_bullet%d_ion%d.png", nBulletNum, 1)
        else
            sBulletName = string.format("dntg_bullet%d_ion%d.png", nBulletNum, 2)
        end
    end
	self:initWithSpriteFrameName(sBulletName)
    local movedis = 100
...
function Bullet:initPhysicsBody()
	...
	self:setPhysicsBody(cc.PhysicsBody:createBox(cc.size(self:getContentSize().width, self:getContentSize().height)))
    self:getPhysicsBody():setCategoryBitmask(2)
    self:getPhysicsBody():setCollisionBitmask(0)
    self:getPhysicsBody():setRotationOffset(0)
...
function Bullet:changeDisplayFrame( chairId , score)
	local nBulletNum = self:getBulletNum()
	local frame = string.format("dntg_bullet%d_norm%d.png", nBulletNum,chairId + 1)
...
function Bullet:sendCathcFish( rect,fishId )        -- 发送捕获消息
	...
	for i = 1, #tmp do     -- 筛选大鱼
		local fish = tmp[i]
		if fish.m_data.fish_kind >= g_var(cmd).FishKind.FISH_YINSHA then
