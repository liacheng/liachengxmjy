...
function GameFrame:ctor()
    ...
    self.m_sinList = {}
    self.m_cosList = {}
    self.exchange_fish_score_ = {0,0,0,0}   -- 每个位置上交换的鱼币
    self.m_UserBulletId_ = {0,0,0,0}
    self.fish_score_ = {0,0,0,0}            -- 捕鱼和交换的总鱼币
    self.bullet_speed_ = {10,20,30,40,50}       -- 子弹的速度
    self.bullet_speed_Mutiple = 1           -- 子弹加速倍数
    self.bullet_speed_index_ = {1,1,1,1}    -- 子弹的档位
    self.bullet_send_ = {0.3,0.3,0.3,0.3,0.3} --self.bullet_send_ = {0.8,0.7,0.6,0.4,0.2}   -- 发射子弹的速度
    self.bullet_update_ = {0.4,0.3,0.25,0.2,0.15}   -- 子弹刷新的速度
    self.m_currentMutiple = 100                 -- 当前自己的子弹倍数
    self.m_userCurrentMutiple = {0,0,0,0}   -- 记录用户的子弹倍数
...
function GameFrame:selectMaxFish(index)     -- 自动锁定搜寻大鱼
	...
	for k,v in pairs(self.m_fishList) do
		if v.m_data.fish_kind > fishtype and cc.rectContainsPoint(rect, cc.p(v:getPositionX(), v:getPositionY())) then
            curIndex = k
            if (index ~= nil and index ~= k) or index == nil then
                break
            end
		end
	end
...
function GameFrame:checkRes()
    ...
    local anim = cc.AnimationCache:getInstance():getAnimation("dntg_22_yd")
