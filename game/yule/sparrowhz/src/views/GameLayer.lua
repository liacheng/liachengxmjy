...
function GameLayer:onEventUserStatus(useritem,newstatus,oldstatus)
   ...
   print("change user " .. useritem.wChairID .. "; nick " .. useritem.szNickName)
    ...
    if newstatus.cbUserStatus == yl.US_FREE or newstatus.cbUserStatus == yl.US_NULL then
        ...
        if (newstatus.wTableID ~= self:GetMeUserItem().wTableID)  or  not self.isSceneMessage then
            return
        end
       ...
       -- if useritem == self:GetMeUserItem() then
            --return
        --end
       -- self._gameView:showUserInfo(useritem,(newstatus.cbUserStatus == yl.US_OFFLINE and true or false))
...
function GameLayer:onEventUserEnter(tableid,chairid,useritem)
  ...
  --刷新用户信息
    if useritem == self:GetMeUserItem() or tableid ~= self:GetMeUserItem().wTableID or not self.isSceneMessage then
        return
    end
...
function GameLayer:onSubGameStart(dataBuffer)
	...
	--开始发牌
	self._gameView:gameStart(cmd_data.cbCardData[1], cbCardCount)
...
function GameLayer:onSubGameConclude(dataBuffer)
	...
	self:runAction(cc.Sequence:create(cc.DelayTime:create(1), cc.CallFunc:create(function(ref)
		local resultLayer = ResultLayer:create(self._gameView):addTo(self._gameView,GameViewLayer.Z_ORDER_ENUM.LAYER_RESULT)
