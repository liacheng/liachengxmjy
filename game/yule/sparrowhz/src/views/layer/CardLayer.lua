...
local posHandCard = {cc.p(282, 600), cc.p(239, 186), cc.p(1271, 0), cc.p(1110, 700)}
	...
	if self.cbCardCount[viewId] ~= 0 then
		...
		for j = 1, self.cbCardCount[viewId] do
			...
			if viewId == cmd.MY_VIEWID then
				...
				if #meData == 0 or not meData then
					break
				else		
					self.cbCardData = clone(meData)
				end
