...
local SetLayer = import(".SetLayer")
...
function GameViewLayer:priGameLayerZorder()
    return GameViewLayer.Z_ORDER_ENUM.PRI_ROOM_INFO
end
...
function GameViewLayer:ctor(scene)
	...
	self._cardLayer = CardLayer:create(self):addTo(self,GameViewLayer.Z_ORDER_ENUM.LAYER_CARD)							--牌图层
    	...
    	self._setLayer = SetLayer:create(self):addTo(self, GameViewLayer.Z_ORDER_ENUM.LAYER_SET)
	--聊天泡泡
	self.chatBubble = {}
	for i = 1 , cmd.GAME_PLAYER do
		local strFile = ""
		...
		self.chatBubble[i] = display.newSprite(strFile, {scale9 = true ,capInsets = cc.rect(0, 0, 204, 68)})
			...
			:move(posChat[i])
			:setVisible(false)
			:addTo(self, GameViewLayer.Z_ORDER_ENUM.LAYER_CHAT_CONTENT)
	end
	...
	--托管覆盖层
	self.spTrusteeCover = cc.Layer:create():setVisible(false):addTo(self, GameViewLayer.Z_ORDER_ENUM.LAYER_TRUSTEE)
	--阴影层
...
function GameViewLayer:initButtons()
	...
	self.spTableBtBg = self:getChildByTag(GameViewLayer.SP_TABLE_BT_BG)
        :setLocalZOrder(self.layoutShield:getLocalZOrder())
		:move(1486, 706)
		:setVisible(false)
	...
	return
     ...
     for k,v in pairs(self.m_sparrowUserItem) do
          if v and v.dwUserID == userItem.dwUserID then
              self.nodePlayer[self._scene:SwitchViewChairID(v.wChairID)]:setVisible(false)
              v = nil
          end
     end
end
...
function GameViewLayer:OnUpdateUser(viewId, userItem,flag,cleanReady)
	...
	if  userItem then
		...
		local strScore = self:numInsertPoint(userItem.lScore)
		 ...
		 if PriRoom and GlobalUserItem.bPrivateRoom then
		            if PriRoom:getInstance().m_tabPriData.cbIsGoldOrGameScore then
		                if  PriRoom:getInstance().m_tabPriData.cbIsGoldOrGameScore == 1 then
		                    ...
		                    self.nodePlayer[viewId]:getChildByTag(GameViewLayer.ASLAB_SCORE):setString(userItem.lScore --[[- PriRoom:getInstance().m_tabPriData.lIniScore]])
		...
		self.nodePlayer[viewId]:removeChildByTag(776)
    		local clipNick = ClipText:createClipText(cc.size(80, 16), userItem.szNickName,appdf.FONT_FILE,14)
    		clipNick:setAnchorPoint(cc.p(0.5, 0.5))
   		clipNick:setPosition(-1, -43)
    		clipNick:setTag(776)
    		self.nodePlayer[viewId]:addChild(clipNick,100)
		--local strNickname = string.EllipsisByConfig(userItem.szNickName, 90, string.getConfig(appdf.FONT_FILE, 14))
...
function GameViewLayer:userChat(wViewChairId, chatString)
	if chatString and #chatString > 0 then
		...
		local labCountLength = cc.Label:createWithTTF(chatString,appdf.FONT_FILE, 24)  
		if labCountLength:getContentSize().width > limWidth then
			self.chatDetails[wViewChairId] = cc.Label:createWithTTF(chatString,appdf.FONT_FILE, 24, cc.size(limWidth, 0))
		else
			self.chatDetails[wViewChairId] = cc.Label:createWithTTF(chatString,appdf.FONT_FILE, 24)
		end
		...
		self.chatDetails[wViewChairId]:addTo(self, GameViewLayer.Z_ORDER_ENUM.LAYER_CHAT_CONTENT)
...
function GameViewLayer:setListeningCard(cbCardData)
	...
	if bTooMany then
		...
		if cbCardCount == 28 then 		--所有牌
			...
			spListenCount = cc.Label:createWithTTF(cbCardCount.."", appdf.FONT_FILE, 30)
		end
		...
		-- cc.Label:createWithTTF("厉害了word哥！你可以胡的牌太多，摆不下了....", appdf.FONT_FILE, 50)
		-- 	:move(260, 40)
		-- 	:setAnchorPoint(cc.p(0, 0.5))
	...
	for i = 1, cbCardCount do
		...
		local cardFont = display.newSprite(strFile)
			:move(width/2, height/2 + 8)
			:addTo(card)
		cc.Label:createWithTTF("1", appdf.FONT_FILE, 16)		--番数
			:move(220 + tempX, 61)
		...
		display.newSprite("#sp_listenTimes.png")
			:move(244 + tempX, 61)
			:addTo(self.spListenBg)
		cc.Label:createWithTTF(nLeaveCardNum.."", appdf.FONT_FILE, 16) 		--剩几张
			:move(220 + tempX, 31)
			:setColor(cc.c3b(254, 246, 165))
			:setTag(cbCardIndex)
			:addTo(self.spListenBg)
...
function GameViewLayer:showOperateFlag(viewId, operateCode)
	...
	display.newSprite(strFile)
		...
		:addTo(self, GameViewLayer.Z_ORDER_ENUM.LISTEN_BG)
...
function GameViewLayer:numInsertPoint(lScore)
    return ExternalFun.formatScoreText(lScore)
--[[	local strRes = ""
	if lScore<0 then
		strRes = ""..lScore
