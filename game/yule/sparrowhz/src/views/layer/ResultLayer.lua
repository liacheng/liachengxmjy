...
function ResultLayer:ctor(scene)
    ...
    --
    self.spMaCard = cc.Node:create():addTo(self):setPosition(btContinue:getPositionX()+130, btContinue:getPositionY()-30)
end
...
function ResultLayer:showLayer(resultList, cbAwardCard, cbRemainCard, wBankerChairId, cbHuCard)
    ...
    local nMaCount = self._scene._scene.cbMaCount
	local width = 44
	local height = 67
	...
	for i = 1, cmd.GAME_PLAYER do
		...
		if i <= #resultList then
			...
			local nick =  ClipText:createClipText(cc.size(101, 23),resultList[i].userItem.szNickName,appdf.FONT_FILE,19);	   
			...
			if resultList[i].cbChHuKind >= GameLogic.WIK_CHI_HU then
				...
				display.newSprite("#sp_ziMo.png")
					:move(fX + 21, fY + 32)
					:addTo(nodeUserCard)
				--local path = nMaCount == 1  and "gameResult/title_ymqz.png" or "gameResult/title_blzm.png"
                local path = "gameResult/title_zm.png"
