...
function GameLogic:DeduceWinner( nWinMultiple,cbDiceValue )
	--临时骰子数据
    local cbDiceMap = {}
    local cbAllDiceValue = 0
	for i=1,3 do
        local val = cbDiceValue[i]
		cbDiceMap[val] = (cbDiceMap[val] or 0) + 1
        cbAllDiceValue = cbAllDiceValue + val
	end
    local nDiceTypeCount = table.nums(cbDiceMap)
	--判断对子和组合
	for cx=1,3 do
		for cy=cx+1,3 do
	...
	--判断骰字面值
	if nDiceTypeCount ~=1 then -- 全骰通杀单双与大小
		if cbAllDiceValue >= 11 and cbAllDiceValue <= 17 then
			nWinMultiple[1] = 1
		end
		if cbAllDiceValue >=4 and cbAllDiceValue <= 10 then
			nWinMultiple[2] = 1
		end
	end
	--单双
	if nDiceTypeCount ~=1 then -- 全骰通杀单双与大小
		if cbAllDiceValue%2 == 0 then
			--print("双")
			nWinMultiple[52]=1
		end
		if cbAllDiceValue%2 == 1 then
			--print("单")
			nWinMultiple[51]=1
		end
	end
	if cbAllDiceValue >= 4 and cbAllDiceValue <= 17 then
		print("cbAllDiceValue",cbAllDiceValue)
		local cbAreaIndex = GameLogic.DiceMutipleEx[cbAllDiceValue-3][1]
		print(cbAreaIndex)
		nWinMultiple[cbAreaIndex]= GameLogic.DiceMutipleEx[cbAllDiceValue-3][2]
		print(nWinMultiple[cbAreaIndex])
	end
	--判断同三，同二，和单个
	for cbVal, cbCount in pairs(cbDiceMap) do
		--最下方6个区域
		nWinMultiple[44+cbVal]=cbCount
		if cbCount == 3 then
			nWinMultiple[8+cbVal]=150
			nWinMultiple[15]=24
		end
	end
end
...
--[[
function GameLogic:DeduceWinner2( nWinMultiple,cbDiceValue )
	--临时骰子数据
