...
function PriGameLayer:onButtonClickedEvent( tag, sender )
    ...
    elseif BTN_INVITE == tag then
        ...
        PriRoom:getInstance():getPlazaScene():popTargetShare(clicktype,function(target, bMyFriend)
            ...
            local shareTxt = "新六人牛牛约战 房间ID:" .. self.m_atlasRoomID:getString() .. " 局数:" .. PriRoom:getInstance().m_tabPriData.dwDrawCountLimit
            local friendC = "新六人牛牛房间ID:" .. self.m_atlasRoomID:getString() .. " 局数:" .. PriRoom:getInstance().m_tabPriData.dwDrawCountLimit
            if bMyFriend then
            ...
            elseif nil ~= target then
                ...
                MultiPlatform:getInstance():shareToTarget(target, sharecall, "【约战房间:" .. self.m_atlasRoomID:getString() .."】", shareTxt .. " 新六人牛牛游戏精彩刺激, 一起来玩吧! ", url, "")
            end
        end)
...
function PriGameLayer:onRefreshInfo()
    -- 房间ID
    self.m_atlasRoomID:setString(PriRoom:getInstance().m_tabPriData.szRoomID or "000000")
...
function PriGameLayer:onPriGameEnd( cmd_table )
    ...
    roomID:setString(PriRoom:getInstance().m_tabPriData.szRoomID or "000000")
    --创建时间
...
--    atlas_config:setString(configStr)
    ...
    for i = 1, chairCount do
        ...
        if nil ~= useritem then
            ...
            local nick =  ClipText:createClipText(cc.size(140, 26),useritem.szNickName,appdf.FONT_FILE,22);
