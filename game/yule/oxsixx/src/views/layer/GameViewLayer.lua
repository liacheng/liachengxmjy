...
function GameViewLayer:onRestart()
	...
	self.btMul = {}
	self.btConfirm:setVisible(false) --隐藏确定
...
function GameViewLayer:ctor(scene)
    ...
    self._chatLayer = GameChatLayer:create(self._scene._gameFrame, tabconfig.csbfile):addTo(self, GameViewLayer.ZORDER_CHAT)	--聊天框
...
function GameViewLayer:onButtonClickedEvent(tag,ref)
	if tag == TAG_ENUM.BT_START then   --开始
		self.btStart:setVisible(false)
...
function GameViewLayer:onButtonConfirm(viewid)
	...
	self.btConfirm:setVisible(false) --隐藏确定框
...
function GameViewLayer:gameCallBanker(callBankerViewId, bFirstTimes)
    ...
    --背景闪烁
    if self.m_tabUserHead[callBankerViewId] then
...
function GameViewLayer:gameAddScore(viewId, score)
    ...
    if nil ~= playerItem then
        playerItem:showCallInScore(score,true)
    end
			...
			if self._scene.cbCardData[i][1] ~= 0 and lCardType[i] then
				...
				elseif lCardType[i] > 10 then --特殊牌型
					...
					if self._scene.m_tabPrivateRoomConfig.cardType == cmd.CARDTYPE_CONFIG.CT_CLASSIC then --经典模式
						if lCardType[i] == GameLogic.CT_CLASSIC_OX_VALUE_FOURFLOWER then --四花牛
							cardType = "ox_11.png"
						elseif lCardType[i] == GameLogic.CT_CLASSIC_OX_VALUE_FIVEFLOWER then --五花牛
							cardType = "ox_12.png"
							soundFile = "niu_wuhua_"
						elseif lCardType[i] == GameLogic.CT_CLASSIC_OX_VALUE_SHUNZI then --顺子
							cardType = "ox_13.png"
						elseif lCardType[i] == GameLogic.CT_CLASSIC_OX_VALUE_SAMEFLOWER then--同花
							cardType = "ox_14.png"
						elseif lCardType[i] == GameLogic.CT_CLASSIC_OX_VALUE_HULU then--葫芦
							cardType = "ox_15.png"
						elseif lCardType[i] == GameLogic.CT_CLASSIC_OX_VALUE_BOMB then--炸弹
							cardType = "ox_16.png"
							soundFile = "niu_sizha_"
						elseif lCardType[i] == GameLogic.CT_CLASSIC_OX_VALUE_TONGHUASHUN then--同花顺
							cardType = "ox_17.png"
						elseif lCardType[i] == GameLogic.CT_CLASSIC_OX_VALUE_FIVESNIUNIU then--五小牛	
							cardType = "ox_18.png"
							soundFile = "niu_5_s_"
						end
					elseif self._scene.m_tabPrivateRoomConfig.cardType == cmd.CARDTYPE_CONFIG.CT_ADDTIMES then --疯狂加倍

						if lCardType[i] == GameLogic.CT_ADDTIMES_OX_VALUE_SHUNZI then --顺子
							cardType = "ox_13.png"
						elseif lCardType[i] == GameLogic.CT_ADDTIMES_OX_VALUE_SAMEFLOWER then --同花
							cardType = "ox_14.png"
						elseif lCardType[i] == GameLogic.CT_ADDTIMES_OX_VALUE_HULU then --葫芦
							cardType = "ox_15.png"
						elseif lCardType[i] == GameLogic.CT_ADDTIMES_OX_VALUE_FOURFLOWER then--四花牛
							cardType = "ox_11.png"
						elseif lCardType[i] == GameLogic.CT_ADDTIMES_OX_VALUE_FIVEFLOWER then--五花牛
							cardType = "ox_12.png"
							soundFile = "niu_wuhua_"
						elseif lCardType[i] == GameLogic.CT_ADDTIMES_OX_VALUE_FIVESNIUNIU then--五小牛
							cardType = "ox_18.png"
							soundFile = "niu_5_s_"
						elseif lCardType[i] == GameLogic.CT_ADDTIMES_OX_VALUE_BOMB then--炸弹
							cardType = "ox_16.png"
							soundFile = "niu_sizha_"
						elseif lCardType[i] == GameLogic.CT_ADDTIMES_OX_VALUE_TONGHUASHUN then--同花顺
							cardType = "ox_17.png"
						end
					end
...
function GameViewLayer:setScoreJetton( tabJetton )
	...
	for i = 1, #tabJetton do
		...
		if tabJetton[i] ~= 0 then
			...
			local label = cc.LabelTTF:create(tabJetton[i], appdf.FONT_FILE, 36, cc.size(150,0), cc.TEXT_ALIGNMENT_CENTER)        
...
function GameViewLayer:setBankerUser(wViewChairId,isAni)
	...
	--庄家提示
	if wViewChairId ~= nil and wViewChairId ~= yl.INVALID_CHAIR then
...
function GameViewLayer:setUserTableScore(wViewChairId, lScore)
    ...
    local playerItem = self.m_tabUserHead[wViewChairId]
    if nil ~= playerItem then
        playerItem:showCallInScore(lScore,true)
    end
...
function GameViewLayer:runWinLoseAnimate(viewid, score)
	...
	--加减
	local labAtNum = cc.LabelAtlas:_create(string.format("/" .. score), strNum, 30, 38, string.byte("/"))		--数字
