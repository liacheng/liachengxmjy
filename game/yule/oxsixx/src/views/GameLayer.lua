...
--    self.m_tabPrivateRoomConfig.betType = cmd_table.btConfig
    ...
    for i = 1, cmd.GAME_PLAYER do
        if self.cbPlayStatus[i] == 1 then --当前玩家没有叫庄
            local wViewChairId = self:SwitchViewChairID(i - 1)
            if cbCallBankerStatus[i] == false then
...
--        PriRoom:getInstance().m_tabPriData.dwPlayCount = curcount
--        if nil ~= self._gameView._priView and nil ~= self._gameView._priView.onRefreshInfo then
--            self._gameView._priView:onRefreshInfo()
--        end
--    end
...
--        print("my score is ===",lTableScore[self:GetMeChairID()])
...
--        end
            local Jettons = {}
            local jetton = self.m_lMaxTurnScore * 2
            for i = 1, 4 do 
                jetton = math.floor(jetton / 2)
                table.insert(Jettons, jetton)
            end
            self._gameView:setScoreJetton(Jettons)
...
--        self.lPercentConfig[i] = cmd_table.lPercentConfig[1][i]
    ...
    for i = 1, cmd.GAME_PLAYER do
        ...
        if self.cbPlayStatus[i] == 1 then
            ...
            if not bOxCard[i] and cmd.MY_VIEWID == wViewChairId then 
...
--    self.cbCardData  = {} 
    ...
    if cmd_table.bFirstTimes then
        --self._gameView.m_tabUserHead[viewid]:hiddenMultiple()
        return
    end
...
--    end
    ...
    --显示庄家标识
    self._gameView:setBankerUser(bankViewId)
    --只显示庄家倍数
...
--        for i=1,cmd.GAME_PLAYER do 
--            local viewid = self:SwitchViewChairID(i-1)
--            self._gameView:setBankerWaitIcon(viewid,true)
--        end
...
function GameLayer:onSubGameEnd(dataBuffer)
    ...
    self:runAction(cc.Sequence:create(cc.DelayTime:create(2),cc.CallFunc:create(callfunc)))
